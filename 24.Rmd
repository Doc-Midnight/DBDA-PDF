...
my_data <- read_csv("~/Desktop/DBDA/data.R/HairEyeColor.csv")
...
p10 <-
  tibble(x = 0:9) %>% 
  ggplot(aes(x = x, y = 0.87 * (dpois(x, lambda = 3.5)) / 
                     max(dpois(x, lambda = 3.5))) ) +
...
# combine and plot!
(p1 + p1 + p1 + p2 + p2 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9 + p10 + p11 + 
                p12 + p13) + 
...
fit24.1 <-
  brm(data = my_data,
      ...
      seed = 24,
      stanvars = stanvars,
      file = "~/Desktop/DBDA/fits/fit24.01")
```
...
bind_rows(
  # define beta[0]
  tibble(x = seq(from = y_log_mean - (4 * y_log_sd), to = y_log_mean + 
        (4 * y_log_sd), length.out = 1e3)) %>%
...
deflections %>% 
  ...
  mutate(`Blue.v.Brown\n(x)\nBlack.v.Blond` = 
                 `Blue - Brown @ Black` - `Blue - Brown @ Blond`) %>% 
...
deflections %>% 
  mutate(`Blue - Brown @ Black` = `E:Blue H:Black` - `E:Brown H:Black`,
         `Blue - Brown @ Blond` = `E:Blue H:Blond` - `E:Brown H:Blond`) %>% 
  mutate(`Blue.v.Brown\n(x)\nBlack.v.Blond` = `Blue - Brown @ Black` - 
                 `Blue - Brown @ Blond`) %>% 
...
fit24.2 <-
  brm(data = my_data,
      ...
      stanvars = stanvars,
      seed = 24,
      file = "~/Desktop/DBDA/fits/fit24.02")
```
...
fit24.3 <-
  brm(data = my_data,
      ...
      stanvars = stanvars,
      seed = 24,
      file = "~/Desktop/DBDA/fits/fit24.03")
```
...
as_draws_df(fit24.2) %>% 
  ...
  stat_halfeye(.width = 0.95, fill = hr[7], color = hr[6]) +
...
fit24.4 <-
  brm(data = my_data,
      ...
      control = list(adapt_delta = 0.999),
      seed = 24,
      file = "~/Desktop/DBDA/fits/fit24.04")
```
