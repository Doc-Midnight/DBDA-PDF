...
You can install the current official version of **brms** in the same way you would any other **R** package (i.e., `install.packages("brms", dependencies = TRUE)`). If you want the current developmental version, you could download it from GitHub by executing the following.
...
p3 <-
        tibble(x = 0:1,
                d = (dbinom(x, size = 1, prob = 0.6)) / max(dbinom(x, 
        size = 1, prob = 0.6))) %>% 
  ggplot(aes(x = x, y = d)) +
...
```{r, message = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/z15N50.csv")
```
...
fit8.1 <-
  brm(data = my_data, 
      ...
      seed = 8,
      file = "~/Desktop/DBDA/fits/fit08.01")
```
...
To summarize a posterior in terms of central tendency, **brms** defaults to the mean value (i.e., the value in the 'Estimate' column of the `print()` output). In many of the other convenience functions, you can also request the median instead. For example, we can use the `robust = TRUE` argument to get the 'Estimate' in terms of the median.

```{r}
posterior_summary(fit8.1, robust = TRUE)
```
...
draws %>% 
  ggplot(aes(x = b_Intercept)) +
  stat_pointinterval(aes(y = 1), point_interval = median_qi, 
        .width = c(0.95, 0.5)) +
  stat_pointinterval(aes(y = 2), point_interval = mode_hdi,  
        .width = c(0.95, 0.5)) +
...
```{r, message = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/z6N8z2N7.csv")
...
fit8.2 <-
  brm(data = my_data, 
      ...
      seed = 8,
      file = "~/Desktop/DBDA/fits/fit08.02")
```
...
long_draws <-
  ...
  mutate(name = factor(name, levels = c("theta_Reginald", "theta_Tony", 
...
fit8.3 <-
  brm(data = my_data, 
      ...
      sample_prior = "only",
      seed = 8,
      file = "~/Desktop/DBDA/fits/fit08.03")
```
...
d_line <-
  tibble(value = c(0.75, 0.286, 0.75 - 0.286),
         name  =  factor(c("theta_Reginald", "theta_Tony", 
        "theta_Reginald - theta_Tony"), 
        levels = levels))
```
...
```{r, fig.width = 6.5, fig.height = 2, message = FALSE, warning = FALSE}
draws %>% 
  rename(theta_Reginald = b_sReginald,
         theta_Tony     = b_sTony) %>% 
  mutate(`theta_Reginald - theta_Tony` = theta_Reginald - theta_Tony) %>% 
...
draws %>% 
  ...
  scale_x_continuous(expression(theta[1]), 
        ...
        breaks = 0:4 / 4, labels = c("0", "0.25", "0.5", "0.75", "1")) +
