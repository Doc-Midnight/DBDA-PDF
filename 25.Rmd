...
```{r, message = F}
fit24.1 <- readRDS("~/Desktop/DBDA/fits/fit24.01.rds")
```
...
```{r}
hdi_of_icdf(qbeta, shape1 = 2, shape2 = 2, width = 0.8)
```
...
**Warning**! Rather than examine both kinds of censoring at once, as in the text, I'm going to slow down and break this section up. First, we'll explore right censoring. Second, we'll explore interval censoring. For our grand finale, we'll combine the two, as in the text.
...
d %>% 
  ...
  ggplot(aes(x = value)) +
  geom_histogram(size = 0.25, binwidth = 2, boundary = 106,
...
fit25.1a <-
  brm(data = d,
      ...
      chains = 4, cores = 4,
      stanvars = stanvars, 
      file = "~/Desktop/DBDA/fits/fit25.01a")
```
...
fit25.2a <-
  brm(data = d,
      ...
      inits = inits_list, 
      file = "~/Desktop/DBDA/fits/fit25.02a")
```
...
fit25.1b <-
  brm(data = d,
      ...
      chains = 4, cores = 4,
      stanvars = stanvars, 
      file = "~/Desktop/DBDA/fits/fit25.01b")
...
fit25.2b <-
  brm(data = d,
      ...
      # here we insert our start values for the intercept
      inits = inits_list, 
      file = "~/Desktop/DBDA/fits/fit25.02b")
```
...
fit25.1c <-
  brm(data = d,
      ...
      chains = 4, cores = 4,
      stanvars = stanvars, 
      file = "~/Desktop/DBDA/fits/fit25.01c")
...
fit25.2c <-
  brm(data = d,
      ...
      # here we insert our start values for the intercept
      inits = inits_list, 
      file = "~/Desktop/DBDA/fits/fit25.02c")
```
...
geom_hist <- function(xintercept = xintercept, binwidth = binwidth, ...) {
  ...
  list(
    geom_histogram(fill = "skyblue", color = "white", size = 0.2, 
                   binwidth = binwidth, boundary = 106),
    geom_vline(xintercept = xintercept, color = "skyblue3", 
               size = 1/2, linetype = 3),
    stat_pointinterval(aes(y = 0), point_interval = mode_hdi, .width = 0.95),
    scale_y_continuous(NULL, breaks = NULL)
  )
...
p2 <-
  ...
  geom_histogram(data = d, 
                 aes(y = stat(density)),
                 size = 0.25, binwidth = 2, boundary = 106,
                 fill = "skyblue4", color = "white") +
  geom_line(aes(y = density, group = .draw),
            size  = 1/4, alpha = 1/3, color = "skyblue") +
  scale_x_continuous("data with posterior predictive lines", 
                     limits = c(40, 110)) +
  scale_y_continuous(NULL, breaks = NULL)
```
...
((p1 | p2) / (p3 | p4) / (p5 | p6) / (p7 | p8)) +
  ...
  caption = expression(atop(italic("Upper quartet")*": Censored data omitted from analysis; parameter estimates are too small.  ", 
  italic("Lower quartet")*": Censored data imputed in known bins; parameter estimates are accurate."))) &
