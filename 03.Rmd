...
```{r, eval = FALSE}
packages <- c("bayesplot", "brms", "coda", "cowplot", "cubelyr", "devtools",
              "fishualize", "GGally", "ggdist", "ggExtra", "ggforce", "ggmcmc",
              "ggridges", "ggthemes", "janitor", "lisa", "loo", "palettetown",
              "patchwork", "psych", "remotes", "rstan", "santoku", "scico",
              "tidybayes", "tidyverse")
...
```{r, fig.width = 3.5, fig.height = 3, message = FALSE, warning = FALSE}
library(tidyverse)
...
### Get the Programs Used in This Book
...
It's handy to know that `TRUE` is a stand in for `TRUE` and ``FALSE is a stand in for `FALSE`. You'll probably notice I use the abbreviations most of the time.
...
x <-
  matrix(1:6, 
       nrow = 2,
       dimnames = list(TheRowDimName = c("row_1_name", "row_2_name"),
                       TheColDimName = c("col_1_name", "col_2_name", 
                                         "col_3_name")))
...
```{r, echo = FALSE, fig.align = "center", out.width = "100%", fig.cap = "Indexing lists in rstats. Inspired by the Residence Inn"}
knitr::include_graphics("~/Desktop/DBDA/pics/pepper_list.png") 
```
...
### Reading CSV Files and Tables with R
...
```{r, message = FALSE}
hgn <- read_csv("~/Desktop/DBDA/data.R/HGN.csv")
```
