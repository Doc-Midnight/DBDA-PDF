...
> This chapter considers data structures that consist of a dichotomous predicted variable. The early chapters of the book were focused on this type of data, but now we reframe the analyses in terms of the generalized linear model$\ldots$
> 
...
\begin{align*}
\mu & = \operatorname{logistic} \left(\beta_0 + \beta_1 x_1 + \beta_2 x_2 \right) \\
...
```{r, warning = FALSE, message = FALSE, fig.width = 4, fig.height = 1}
library(PNWColors)
...
```{r, fig.width = 4, fig.height = 4.25, warning = FALSE, message = FALSE}
library(patchwork)
...
p1 <-
  ...
  annotate(geom = "text",
           ...
           size = 7, color = pm[1], hjust = 0, family = "Times", 
            parse = TRUE) +
  scale_x_continuous(expand = c(0, 0)) +
  theme_void() +
  theme(axis.line.x = element_line(size = 0.5),
        plot.background = element_rect(fill = pm[8], color = "white", 
          size = 1))
...
my_arrow <- arrow(angle = 20, length = unit(0.35, "cm"), 
  type = "closed")
p3 <-
  tibble(x    = 0.5,
         y    = 1,
  ...
  annotate(geom = "text",
           x = 0.55, y = 0.4,
           label = "'~'",
           size = 10, color = pm[1], family = "Times", 
            parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
p4 <-
  ...
  annotate(geom = "text",
           x = c(0.3, 0.48), y = 0.4,
           label = c("'~'", "italic(j)"),
           size = c(10, 7), color = pm[1], family = "Times", 
            parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
p5 <-
  tibble(x = 0.5, y = 0.5,
  label = "logistic(beta[0]+sum()[italic(j)]~beta[italic(j)]~
    italic(x)[italic(ji)])") %>% 
  
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = 7, color = pm[1], parse = TRUE, family = "Times") +
...
p6 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = c(10, 7), color = pm[1], parse = TRUE, 
    family = "Times") +
...
p7 <-
  ...
  annotate(geom = "text",
           x = 0.5, y = 0.94,
           label = "mu[italic(i)]", 
           size = 7, color = pm[1], family = "Times", parse = TRUE) +
  xlim(-.75, 1.75) +
  theme_void() +
  theme(axis.line.x = element_line(size = 0.5),
...
p8 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = c(10, 7), color = pm[1], parse = TRUE, family = "Times") +
  geom_segment(x = 0.5, xend = 0.5,
               y = 1, yend = 0, 
...
my_data <- read_csv("~/Desktop/DBDA/data.R/HtWtData110.csv")
...
fit21.1 <-
  brm(data = my_data, 
      ...
      seed = 21,
      file = "~/Desktop/DBDA/fits/fit21.01")
```
...
draws %>% 
  ...
  # plot!
  ggplot(aes(x = weight)) +
  geom_hline(yintercept = 0.5, color = pm[7], size = 1/2) +
...
draws <-
  draws %>% 
  # convert the parameter draws to their natural metric following 
  # Equation 21.1 (pp. 624--625)
  transmute(Intercept = b_Intercept - (b_weight_z * 
      mean(my_data$weight) / sd(my_data$weight)),
...
fit21.2 <-
  brm(data = my_data, 
      ...
      iter = 2500, warmup = 500, chains = 4, cores = 4,
      seed = 21,
      file = "~/Desktop/DBDA/fits/fit21.02")
```
...
draws <-
  ...
  mutate(b_weight  = b_weight_z / sd(my_data$weight),
         ...
         Intercept = b_Intercept - ((b_weight_z * mean(my_data$weight) / 
             sd(my_data$weight)) +(b_height_z * mean(my_data$height) / 
                 sd(my_data$height)))) %>% 
...
draws %>% 
  ...
  mutate(height = (-Intercept / b_height) + 
      (-b_weight / b_height) * weight) %>% 
...
```{r, fig.width = 8, fig.height = 2.5, warning = FALSE, message = FALSE}
library(stringr)
draws %>% 
  pivot_longer(-.draw) %>% 
...
$$y \sim \operatorname{dbern}(m), m = \operatorname{logistic} \left(0.018 x_1 + 0.7 x_2 - 50 \right).$$
...
```{r, warning = FALSE}
as_draws_df(fit21.2) %>% 
  transmute(beta_0 = b_Intercept - ((b_weight_z * mean(my_data$weight) / 
      sd(my_data$weight)) + ((b_height_z * mean(my_data$height) / 
          sd(my_data$height)))),
...
Thus, "the ratio, $p(y = 1) / p(y = 0)$, is called *the odds of outcome 1 to outcome 0*, and therefore $\operatorname{logit}(\mu)$ is the log odds of outcome 1 to outcome 0" (p. 630; *emphasis* mine).
...
fit21.3 <-
  brm(data = d_rare, 
      ...
      iter = 2500, warmup = 500, chains = 4, cores = 4,
      seed = 21,
      file = "~/Desktop/DBDA/fits/fit21.03")
```
...
draws %>% 
  ...
  geom_point(data = d_rare,
             aes(y = y),
             alpha = 1/3, color = pm[1]) +
  ggtitle("Data with Posterior Prediction") +
...
fit21.4 <-
  update(fit21.3,
         ...
         iter = 2500, warmup = 500, chains = 4, cores = 4,
         seed = 21,
         file = "~/Desktop/DBDA/fits/fit21.04")
```
...
fit21.5 <-
  brm(data = d, 
      ...
      iter = 2500, warmup = 500, chains = 4, cores = 4,
      seed = 21,
      file = "~/Desktop/DBDA/fits/fit21.05")
```
...
```{r, fig.width = 6, fig.height = 1, warning = FALSE}
as_draws_df(fit21.5) %>% 
...
```{r, fig.width = 4, fig.height = 3.75, warning = FALSE, message = FALSE}
as_draws_df(fit21.5) %>% 
...
fit21.6 <-
  brm(data = d, 
      ...
      iter = 2500, warmup = 500, chains = 4, cores = 4,
      seed = 21,
      file = "~/Desktop/DBDA/fits/fit21.06")
```
...
>
> $$\mu = \alpha \cdot \frac{1}{2} + (1 - \alpha) \cdot \operatorname{logistic} \left( \beta_0  + \sum_j \beta_j x_j \right)$$
>
...
fit21.7 <-
  brm(data = my_data, 
      ...
      iter = 2500, warmup = 500, chains = 4, cores = 4,
      seed = 21,
      file = "~/Desktop/DBDA/fits/fit21.07")
```
...
p1 <- 
  ...
  mutate(male   = 0.5 * b_a_Intercept + (1 - b_a_Intercept) * inv_logit_scaled(b_b0_Intercept + b_b1_Intercept * weight_z),
         weight = weight_z * sd(my_data$weight) + mean(my_data$weight),
         thresh = -b_b0_Intercept / b_b1_Intercept * 
      sd(my_data$weight) + mean(my_data$weight)) %>% 
  ...
  geom_point(data = my_data,
             alpha = 1/3, color = pm[1]) +
  labs(title = "Data with Posterior Prediction", 
       y = "male") +
  coord_cartesian(xlim = range(my_data$weight))
```
...
p2 <-
  draws %>% 
  mutate(Intercept = b_b0_Intercept - (b_b1_Intercept * 
      mean(my_data$weight) / sd(my_data$weight)),
         weight    = b_b1_Intercept / sd(my_data$weight),
         guessing  = b_a_Intercept) %>% 
  pivot_longer(Intercept:guessing) %>% 
  mutate(name = factor(name, levels = c("Intercept", "weight", 
    "guessing"))) %>% 
...
$$\beta_0 \sim \operatorname{Normal} \left( M_0, S_0 \right) \; .$$

To further explore what this means, we might make the model diagram in Figure 21.10.

```{r, fig.width = 2.5, fig.height = 4.25, warning = FALSE, message = FALSE}
p1 <-
  tibble(x = seq(from = -3, to = 3, by = 0.1)) %>% 
  ggplot(aes(x = x, y = (dnorm(x)) / max(dnorm(x)))) +
  geom_area(fill = pm[5]) +
...
p2 <-
  ...
  annotate(geom = "text",
           x = 0.4, y = 0.4,
           label = "'~'",
           size = 10, color = pm[1], family = "Times", parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
p4 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = 10, color = pm[1], parse = TRUE, family = "Times") +
  geom_segment(x = 0.5, xend = 0.5,
               y = 1, yend = 0,
...
p2 <-
  ...
  theme_void() +
  theme(axis.line.x = element_line(size = 0.5),
        plot.background = element_rect(fill = pm[8], 
          color = "white", size = 1))
...
p5 <-
  tibble(x = 0.5,
         y = 0.3,
         label = "logistic(beta[0]+sum()[italic(j)]*
    beta['['*italic(j)*']']*italic(x)['['*italic(j)*']'](italic(i)))") %>% 
...
p6 <-
  ...
  annotate(geom = "text",
           ...
           label = c("'~'", "'~'", "italic(j)"),
           size = c(10, 10, 7), color = pm[1], family = "Times", 
            parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
tibble(x = 0:7) %>% 
  ggplot(aes(x = x, 
             y = (dbinom(x, size = 7, prob = 0.625)) / 
              max(dbinom(x, size = 7, prob = 0.625)))) +
  geom_col(fill = pm[5], width = 0.4) +
  annotate(geom = "text",
           x = 3.5, y = 0.2,
           label = "binomial",
...
\beta_\text{PriPos}         & \sim \operatorname{Normal} \left( 0, \sigma_\text{PriPos} \right) \\
\beta_\text{PriPos:Player}  & \sim \operatorname{Normal} \left( 0, \sigma_\text{PriPos:Player} \right) \\
...
fit21.8 <-
  brm(data = my_data,
      ...
      file = "~/Desktop/DBDA/fits/fit21.08")
```
...
```{r, fig.width = 8, fig.height = 2.75, warning = FALSE, message = FALSE}
library(ggridges)
...
nd %>% 
  ...
  # plot!
  ggplot(aes(y = PriPos)) +
  geom_ridgeline(aes(x = theta, height = -density, 
        group = interaction(PriPos, .draw)),
        fill = NA, color = adjustcolor(pm[1], alpha.f = 1/3),
        size = 1/3, alpha = 2/3, min_height = NA) +
  geom_jitter(data = my_data,
              aes(x = Hits / AtBats, size = AtBats),
              height = 0.05, alpha = 1/6, shape = 1, color = pm[1]) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        ...
        legend.position = c(0.956, 0.8))
```

**Warning**: Since we didn't actually presume the beta distribution anywhere in our `brm()` statistical model, *I would not attempt to present this workflow in a scientific outlet*. Go with the previous plot. This attempt seems dishonest. But it is kinda fun to see how far we can push our results. 
...
p1 <-
  ...
  mutate(name = factor(name, 
    levels = c("Pitcher vs. Catcher", "Catcher vs. 1st Base"))) %>% 
...
p2 <-
  ...
  mutate(`Pitcher vs. Catcher`  = Pitcher - Catcher,
         `Catcher vs. 1st Base` = Catcher - `1st Base`) %>% 
  pivot_longer(contains("vs.")) %>% 
  mutate(name = factor(name, levels = c("Pitcher vs. Catcher", 
        "Catcher vs. 1st Base"))) %>% 
