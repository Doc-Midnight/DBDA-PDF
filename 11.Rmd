...
```{r}
sample(c("H", "T"), size = 24, replace= TRUE)
```
...
h_counter <- function(seed) {
  set.seed(seed)
  coins <- sample(c("H", "T"), size = 24, replace= TRUE)
...
set.seed(115)
tibble(flips = sample(c("H", "T"), size = 24, replace= TRUE)) %>% 
...
>
> $$p \text{ value} = p \left( D_{\theta, I} \succcurlyeq D_\text{actual} \mid \theta, I \right)$$
>
...
p1 <-
  ...
  geom_tile(color = alpha(jj[2], 0.25)) +
...
>
> \begin{align*}
> p \left( N \mid z, \theta \right) &= \binom{N - 1}{z - 1} \theta^{z-1} (1 - \theta)^{N - z} \cdot \theta \\
> &= \binom{N - 1}{z - 1} \theta^z (1 - \theta)^{N - z} \\
...
```{r, fig.width = 5, fig.height = 3.25, warning= FALSE}
flips %>%
...
We got the formula for that last variable, `p \left(N \mid z,theta \right)`, from Formula 11.6 on page 306. You'll note how Kruschke continued to refer to it as $p \left( z \mid N \right)$ in his Figure 11.4. It's entirely opaque, to me, how $p \left( z \mid N \right) = p \left( N \mid z, \theta \right)$. I'm just going with it.
...
flips %>% 
  ...
  summarise(`one-sided p-value` = sum(`p(N|z,theta)`) %>% round(digits = 3),
            `two-sided p-value` = (2 * sum(`p(N|z,theta)`)) %>% 
                    round(digits = 3))
```
...
$$p_\text{figure 11.6} = 1 - \left(1 - p_\text{figure 11.3} \right)^2 \;.$$
...
Nothing for us here (**brms** does not do NHST analysis).
...
```{r}
hdi_of_icdf <- function(name = qbeta, width = 0.95, tol = 1e-8, ... ) {
...
## The Confidence Interval and the Highest Density Interval
...
Figure 11.8 depicts the sampling distributions for $\theta = 0.126$ (top row) and $\theta = 0.511$ (bottom row). Here's the upper- and lower-left panels of Figure 11.8.
...
p2 <-
  ...
  geom_col(fill = jj[1]) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta=="0.511"))
...
p1 <-
  tibble(z = 0:24,
         y = dbinom(0:24, size = 24, prob = 0.126)) %>% 
...
p2 <-
  tibble(z = 0:24,
       y = dbinom(0:24, size = 24, prob = 0.511)) %>% 
...
p1 <-
  ...
  geom_col(fill = jj[1]) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta=="0.126")) +
...
p2 <-
  ...
  geom_col(fill = jj[1]) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta=="0.484")) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), limits = c(0, 1)) +
  theme(title = element_text(size = 10),
        axis.ticks.x = element_blank())
```
...
Figure 11.10 is a depiction of when an experimenter intended to stop when a fixed duration expired. This time the two rows are based on $\theta = 0.135$ (top row) and $\theta = 0.497$ (bottom row). We'll follow the same general procedure from the last figure. Here are the subplots on the left.
...
p1 <-
  ...
  scale_y_continuous(expand = expansion(mult = 0), limits = c(0, 1)) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta=="0.135")) +
...
p2 <-
  ...
  scale_y_continuous(expand = expansion(mult = 0), limits = c(0, 1)) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta=="0.497")) +
  coord_cartesian(ylim = c(0, 1)) +
  theme(axis.ticks.x = element_blank())
```
...
p5 <- tibble(x = 1.5,
  ...
  geom_text(aes(size = size), 
            parse= TRUE, hjust = 0.5, show.legend = F) +
...
```{r, fig.width = 2.5, fig.height = 6, message= FALSE}
p1 <-
  tibble(y      = factor(c("tail", "head"), levels = c("tail", "head")),
         `p(y)` = c(1 - 0.11, 0.11)) %>% 
  ...
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta[1] == "0.11;"*~theta[2] == "0.11"))
...
p2 <-
  ...
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta[1] == "0.539;"*~theta[2] == "0.539"))
...
At this point, it's worth quoting Kruschke at length:
...
### NHST Correction for Experimentwise Error
...
Here's what this looks like in when $\alpha_\text{PC} = 0.05$ and $c = 36$.
...
Here's how to apply the Bonferroni correction to our example if the desired false-alarm rate is 0.05.
...
```{r, fig.width = 5, fig.height = 3.5}
# TK add scales library for plotting
library(scales)
...
tibble(c = 1:100) %>% 
  ...
  scale_x_continuous(expand = expansion(mult = c(0, 0.05)), 
    limits = c(0, NA)) +
  # TK convert vertical axis to exponential scale
  scale_y_log10("y",
...
### There's Just One Bayesian Posterior No Matter How You Look at It
...
"Sampling distributions tell us the probability of imaginary outcomes given a parameter value and an intention, $p \left( D_{\theta, I} \mid \theta, I \right)$, instead of the probability of parameter values given the actual data, $\left( \theta \mid D_\text{actual} \right)$."
