...
We got the formula for that last variable, `p(N \mid z,theta)`, from Formula 11.6 on page 306. You'll note how Kruschke continued to refer to it as $p(z \mid N)$ in his Figure 11.4. It's entirely opaque, to me, how $p(z \mid N) = p(N \mid z, \theta)$. I'm just going with it.
...
p5 <- tibble(x = 1.5,
  ...
  geom_text(aes(size = size), 
            parse = T, hjust = 0.5, show.legend = F) +
...
p1 <-
  ...
  coord_cartesian(ylim = c(0, 1)) +
  labs(title = "Hypothetical Population",
       subtitle = expression(theta[1]==".11;"*~theta[2]=="0.11"))
