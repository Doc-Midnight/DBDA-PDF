...
d1 <- read_csv("~/Desktop/DBDA/data.R/SoftmaxRegData1.csv")
d2 <- read_csv("~/Desktop/DBDA/data.R/SoftmaxRegData2.csv")
...
\begin{align*}
\phi_{S^* \mid S} = \operatorname{logistic}(\lambda_{S^* \mid S}) \\
\lambda_{S^* \mid S} = \beta_{0, S^* \mid S} + \beta_{1, {S^* \mid S}} x,
\end{align*}

where the conditional response probability (i.e., the goal of the analysis) is $\phi_{S^* \mid S}$. $S^*$ and $S$ denote the subset of outcomes and larger set of outcomes, respectively, and $\lambda_{S^* \mid S}$ is the propensity based on some linear model. The overall point is these "regression coefficients refer to the conditional probability of outcomes for the designated subsets, not necessarily to a single outcome among the full set of outcomes" (p. 655).
...
```{r, warning = F, message = F}
d3 <- read_csv("~/Desktop/DBDA/data.R/CondLogistRegData1.csv")
d4 <- read_csv("~/Desktop/DBDA/data.R/CondLogistRegData2.csv")
...
The divisions in each of these levels can be expressed as linear models which we will denote $\lambda$. Given our data with two predictors `X1` and `X2`, we can express the three linear models as

```{=latex}
\begin{align*}
\lambda _{ \{ 1 \} \mid \{ 1, 2, 3, 4 \}} &= \beta _{ 0, \{ 1 \} \mid \{ 1, 2, 3, 4 \}} + \beta _{ 1, \{ 1 \} \mid \{ 1, 2, 3, 4 \}} \text{X1} + 
\beta _{2, \{ 1 \} \mid \{ 1, 2, 3, 4 \}} \text{X2} \\
%
\lambda _{ \{ 2 \} \mid \{ 2, 3, 4 \}} &= \beta _{ 0, \{ 2 \} \mid \{ 2, 3, 4 \}} + 
\beta _{1, \{ 2 \} \mid \{ 2, 3, 4 \}} \text{X1} + \beta _{ 2, \{ 2 \} \mid \{ 2, 3, 4 \}} \text{X2} \\
%
\lambda _{ \{ 3 \} \mid \{ 3, 4 \}} &= \beta _{ 0, \{ 3 \} \mid \{ 3, 4 \}} + 
\beta _{ 1, \{ 3 \} \mid \{ 3, 4 \}} \text{X1} + \beta _{ 2, \{ 3 \} \mid \{ 3, 4 \}} 
\text{X2} ,
\end{align*}
```
...
```{=latex}
\begin{align*}
\phi_{\{ 1 \} \mid \{ 1, 2, 3, 4 \}} & = \operatorname{logistic} \left (\lambda_{\{ 1 \} \mid \{ 1, 2, 3, 4 \}} \right) \\
\phi_{\{ 2 \} \mid \{ 2, 3, 4 \}}   & = \operatorname{logistic} \left (\lambda_{\{ 2 \} \mid \{ 2, 3, 4 \}} \right) \\
\phi_{\{ 3 \} \mid \{ 3,4 \}}     & = \operatorname{logistic} \left (\lambda_{\{ 3 \} \mid \{ 3, 4 \}} \right),
\end{align*}
```

where $\phi_{\{ 1 \} \mid \{ 1, 2, 3, 4 \}}$ through $\phi_{\{ 3 \} \mid \{ 3, 4 \}}$ are the conditional probabilities for the outcome sets. If, however, we want the conditional probabilities for the actual levels of the criterion `Y`, we define those with a series of (in this case) four equations:

```{=latex}
\begin{align*}
\phi_1 & = \phi_{\{ 1 \} \mid \{ 1,2,3,4 \}} \\
\phi_2 & = \phi_{\{ 2 \} \mid \{ 2,3,4 \}} \cdot \left ( 1 - \phi_{\{ 1 \} \mid \{ 1,2,3,4 \}} \right) \\
\phi_3 & = \phi_{\{ 3 \} \mid \{ 3,4 \}} \cdot \left ( 1 - \phi_{\{ 2 \} \mid \{ 2,3,4 \}} \right) \cdot \left ( 1 - \phi_{\{ 1 \} \mid \{ 1,2,3,4 \}} \right) \\
\phi_4 & = \left ( 1 - \phi_{\{ 3 \} \mid \{ 3,4 \}} \right) \cdot \left ( 1 - \phi_{\{ 2 \} \mid \{ 2,3,4 \}} \right) \cdot \left ( 1 - \phi_{\{ 1 \} \mid \{ 1,2,3,4 \}} \right),
\end{align*}
```
...
It still might not be clear how the various $\phi_{S^* \mid S}$ values connect to the data. Though not in the text, here's an alternative way of expressing the relations in Figure 22.3. This time the plot is faceted by the three levels of $\phi_{S^* \mid S}$ and the background fill is based on those conditional probabilities.
...
Notice how because each of the levels of $\phi$ is defined by a different subset of the data, each of the facets contains a different subset of the `d3` data, too. For example, since $\phi_{\{ 1 \} \mid \{ 1,2,3,4 \}}$ is defined by the full subset of the possible values of `Y`, you see all the `Y` data displayed by `geom_text()` for that facet. In contrast, since $\phi_{\{ 3 \} \mid \{ 3,4 \}}$ is defined by a subset of the data for which `Y` is only `3` or `4`, those are the only values you see displayed within that facet of the plot.
...
```{=latex}
\begin{align*}
\lambda_{\{ 1,2 \} \mid \{ 1,2,3,4 \}} & = \beta_{0,\{ 1,2 \} \mid \{ 1,2,3,4 \}} + \beta_{1,\{ 1,2 \} \mid \{ 1,2,3,4 \}} \text{X1} + \beta_{2,\{ 1,2 \} \mid \{ 1,2,3,4 \}} \text{X2} \\
\lambda_{\{ 1 \} \mid \{ 1,2 \}}   & = \beta_{0,\{ 1 \} \mid \{ 1,2 \}} + \beta_{1,\{ 1 \} \mid \{ 1,2 \}} \text{X1} + \beta_{2,\{ 1 \} \mid \{ 1,2 \}} \text{X2} \\
\lambda_{\{ 3 \} \mid \{ 3,4 \}}     & = \beta_{0,\{ 3 \} \mid \{ 3,4 \}} + \beta_{1,\{ 3 \} \mid \{ 3,4 \}} \text{X1} + \beta_{2,\{ 3 \} \mid \{ 3,4 \}} \text{X2}.
\end{align*}
```
...
```{=latex}
\begin{align*}
\phi_{\{ 1,2 \} \mid \{ 1,2,3,4 \}} & = \operatorname{logistic} \left (\lambda_{\{ 1,2 \} \mid \{ 1,2,3,4 \}} \right) \\
\phi_{\{ 1 \} \mid \{ 1,2 \}}   & = \operatorname{logistic} \left (\lambda_{\{ 1 \} \mid \{ 1,2 \}} \right) \\
\phi_{\{ 3 \} \mid \{ 3,4 \}}     & = \operatorname{logistic} \left (\lambda_{\{ 3 \} \mid \{ 3,4 \}} \right).
\end{align*}
```
...
```{=latex}
\begin{align*}
\phi_1 & = \phi_{\{ 1 \} \mid \{ 1,2 \}} \cdot \phi_{\{ 1,2 \} \mid \{ 1,2,3,4 \}} \\
\phi_2 & = \left ( 1 - \phi_{\{ 1 \} \mid \{ 1,2 \}} \right) \cdot \phi_{\{ 1,2 \} \mid \{ 1,2,3,4 \}} \\
\phi_3 & = \phi_{\{ 3 \} \mid \{ 3,4 \}} \cdot \left ( 1 - \phi_{\{ 1,2 \} \mid \{ 1,2,3,4 \}} \right) \\
\phi_4 & = \left ( 1 - \phi_{\{ 3 \} \mid \{ 3,4 \}} \right) \cdot \left ( 1 - \phi_{\{ 1,2 \} \mid \{ 1,2,3,4 \}} \right),
\end{align*}
```
...
Here's an alternative way of expression the relations in the right panel of Figure 22.3. This time the plot is faceted by the three levels of $\phi_{S^* \mid S}$ and the background fill is based on those conditional probabilities.
...
p7 <-
  tibble(x = 0:3,
         d = c(0.5, 0.85, 0.5, 0.85)) %>% 
  ggplot(aes(x = x, y = d)) +
  geom_col(fill = pl[6], color = pl[5], width = 0.45) +
  annotate(geom = "text",
           x = 1.5, y = 0.2,
           label = "categorical",
           size = 7, color = pl[1]) +
...
p7 <-
  ...
  ggplot(aes(x = x, y = d)) +
  geom_col(fill = pl[6], color = pl[5], width = 0.4) +
  annotate(geom = "text",
           x = 0.5, y = 0.2,
           label = "Bernoulli",
           size = 7, color = pl[1]) +
...
fit22.1 <-
  brm(data = d1, 
      ...
      seed = 22,
      file = "~/Desktop/DBDA/fits/fit22.01")
```
...
fit22.2 <-
  brm(data = d1, 
      ...
      iter = 2000, warmup = 1000, cores = 4, chains = 4,
      seed = 22,
      file = "~/Desktop/DBDA/fits/fit22.02")
```
...
p2 <-
  ...
  scale_y_continuous(NULL, labels = scales::percent, breaks = 0:4 / 10,
                     ...
                     limits = c(0, 0.4)) +
  labs(
    subtitle = "model-based population percentages\n(with 95% interval bars)")
...
p3 <-
  ...
  ggplot(aes(x = Y, y = p)) +
  stat_ccdfinterval(.width = 0.95, fill = pl[6], color = pl[2],
...
text <-
 fitted(fit22.2)[1, , ] %>% 
  ...
  mutate(y       = (Estimate / 2) + lag(cumsum(Estimate), default = 0),
         summary = str_c(round(100 * Estimate, 1), "*'% ± '*", 
                         round(200 * Est.Error, 1)),
         label_y = str_c("Y = ", 1:4))
...
fit22.3 <-
  brm(data = d3, 
      ...
      file = "~/Desktop/DBDA/fits/fit22.03")
```
...
* `mu2_` has to do with $\lambda_{\{ 1 \} \mid \{ 1,2,3,4 \}}$,
* `mu3_` has to do with $\lambda_{\{ 2 \} \mid \{ 2,3,4 \}}$, and
* `mu4_` has to do with $\lambda_{\{ 3 \} \mid \{ 3,4 \}}$.
...
p1 <-
  ...
  ggplot(aes(x = value, y = 0)) +
  stat_histinterval(point_interval = mode_hdi, .width = 0.95,
                    fill = pl[4], slab_color = pl[3], color = pl[1], 
                    point_color = pl[2],
                    normalize = "panels") +
  scale_y_continuous(NULL, breaks = NULL) +
  xlab("marginal posterior") +
...
fit22.4 <-
  brm(data = d4, 
      ...
      file = "~/Desktop/DBDA/fits/fit22.04")
```
...
fit22.5 <-
  brm(data = d3, 
      ...
      iter = 2000, warmup = 1000, cores = 4, chains = 4,
      seed = 22,
      file = "~/Desktop/DBDA/fits/fit22.05")
```
...
bind_rows(
  ...
  geom_vline(xintercept = 0, color = "white") +
  stat_pointinterval(.width = 0.95, point_size = 1.5, size = 1) +
...
```{=latex}
\begin{equation}
x_2 = \left( {\color{red}+} \frac{\beta _0}{\beta _2} \right) + \left( \frac{\beta _1}{\beta _2} \right) x_1 .
\end{equation}
```
...
fit22.6 <-
  brm(data = d4, 
      ...
      iter = 2000, warmup = 1000, cores = 4, chains = 4,
      seed = 22,
      file = "~/Desktop/DBDA/fits/fit22.06")
```
