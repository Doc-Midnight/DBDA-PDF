...
> This chapter considers data structures that consist of a metric predicted variable and two (or more) nominal predictors. This chapter extends ideas introduced in the previous chapter, so please read the previous chapter if you have not already$\ldots$
>
...
>
> \begin{align*}
> \sum_j \beta_{1[j]} = 0 \; , &&&&& \sum_k \beta_{2[k]} = 0 \; ,  \\
...
(
  ...
  mutate(mu_additive = grand_mean + (x1 * deflection_1) + 
           (x2 * deflection_2)) %>% 
...
Similar to the last chapter, our overall plot theme will be based on the default `theme_grey()` with a good number of adjustments. This time, it will have more of a [`theme_black()`](https://jonlefcheck.net/2013/03/11/black-theme-for-ggplot2-2/) vibe. 
...
theme_set(
  theme_grey() +
    theme(text = element_text(color = bd[3]),
          ...
          strip.background = element_rect(fill = alpha(bd[5], 1/3), 
                color = alpha(bd[5], 1/3)),
...
p <-
  ...
  geom_col(aes(fill = x2_c),
           position = "dodge") +
  scale_fill_manual(NULL, values = bd[c(11, 6)], 
                    labels = c("x2[1]", "x2[2]")) +
...
```{r, fig.height = 3, fig.width = 8.25, warning = FALSE, message = FALSE}
...
p3 <-
  ...
  annotate(geom = "text",
           ...
           size = 7, color = bd[3], family = "Times", parse = TRUE) +
...
p8 <-
  tibble(x = 0.5, y = 0.25,
  label = "beta[0]+sum()[italic(j)]*beta[1]['['*italic(j)*']']*
  italic(x)[1]['['*italic(j)*']'](italic(i))+sum()[italic(k)]*
  beta[2]['['*italic(k)*']']*italic(x)[2]['['*italic(k)*']'](italic(i))+
  sum()[italic(jk)]*beta[1%*%2]['['*italic(jk)*']']*
  italic(x)[1%*%2]['['*italic(jk)*']'](italic(i))") %>% 
 
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(hjust = 0.5, size = 7, color = bd[3], parse = TRUE, 
            family = "Times") +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1)) +
  ylim(0, 1) +
  theme_void()
...
p11 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = 10, color = bd[3], parse = TRUE, family = "Times") +
...
p13 <-
  ...
  geom_text(size = c(10, 7), 
            color = bd[3], parse = TRUE, family = "Times") +
  geom_segment(x = 0.5, xend = 0.5,
               y = 1, yend = 0, 
               arrow = my_arrow, color = bd[3]) +
  xlim(0, 1) +
...
p14 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = 7, color = bd[3], parse = TRUE, family = "Times") +
  xlim(0, 1) +
  theme_void()
...
# combine and plot!
(p1 + p1 + p1 + p3 + p4 + p5 + p6 + p2 + p2 + p2 + p8 + p7 + p9 + p10 + 
    p11 + p12 + p13 + p14) + 
...
```{r, message = FALSE, warning = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/Salary.csv")
...
```{r, fig.width = 9, fig.height = 2.5, message = FALSE}
my_data %>% 
  group_by(Pos, Org) %>% 
  ...
  scale_fill_gradient(low = bd[9], high = bd[12],
                      breaks = c(55000, 150000, 260000), 
                      labels = c("$55K", "$150K", "$260K")) +
  scale_x_discrete("Organization", expand = c(0, 0)) +
  scale_y_discrete("Position", expand = c(0, 0)) +
...
fit20.1 <-
  brm(data = my_data,
      ...
      control = list(adapt_delta = 0.999,
                     max_treedepth = 13),
      stanvars = stanvars,
      file = "~/Desktop/DBDA/fits/fit20.01")
``` 
...
```{r, warning = FALSE, message = F ,fig.height = 5.5, fig.width = 8}
library(ggridges)
...
f %>% 
  ...
  geom_jitter(data = my_data %>% 
    ...
    mutate(Org = factor(Org, levels = c("BFIN", "CHEM", "PSY", "ENG"))),
    height = 0.025, alpha = 1/2, size = 2/3, color = bd[11]) +
...
as_draws_df(fit20.1) %>% 
  select(b_Intercept, 
         ...
         sigma) %>% 
  summarise_draws(mean, median, mode = Mode, ess_bulk, 
                  ess_tail, ~ quantile(.x, probs = c(0.025, 0.975))) %>% 
...
As Kruschke reminds us that this parameter is held constant across all subgroups. That is, the subgroups are homogeneous with respect to their variances. We'll relax this constraint later on.
...
stat_beedrill <- function(point_size = 3, 
  ...
  stat_dotsinterval(point_interval = mode_hdi, .width = 0.95, 
                    ...
                    # https://github.com/mjskay/ggdist/issues/93
                    justification = -0.04,
                    ...)
}
```
...
# save the levels
levels <- c("Full - Assis @ PSY", "Full - Assis @ CHEM", 
            "Full.v.Assis\n(x)\nCHEM.v.PSY")
...
f2 <-
  ...
  mutate(`Full.v.Assis\n(x)\nBFIN.v.the rest` = `Full - Assis @ BFIN` - 
           (`Full - Assis @ CHEM` + `Full - Assis @ ENG` + 
              `Full - Assis @ PSY`) / 3)
...
f1 %>% 
  pivot_longer(-contains("_")) %>% 
  mutate(name = factor(name, levels = c("Full - Assis @ PSY", 
        "Full - Assis @ CHEM", "Full.v.Assis\n(x)\nCHEM.v.PSY"))) %>% 
  group_by(name) %>% 
  mode_hdi(value) %>% 
  select(name:.upper) %>% 
...
## Rescaling can Change Iinteractions, Homogeneity, and Normality
...
> Suppose one condition has data values of 100, 110, and 120, while a second condition has data values of 1100, 1110, and 1120. For both conditions, the variance is 66.7, so there is homogeneity of variance. When the data are logarithmically transformed, the variance of the first group becomes $1.05 \times 10^{-3}$, but the variance of the second group becomes two orders of magnitude smaller, namely $1.02 \times 10^{-5}$. In the transformed data there is not homogeneity of variance (p. 601).
...
```{r, fig.width = 8, fig.height = 7.25, message = FALSE}
# bracket
p1 <-
  tibble(x     = 0.99,
         y     = 0.5,
...
p8 <-
  tibble(x = 0.5,
         ...
         label = "beta[0]+sum()[italic(j)]*beta[1]['['*italic(j)*']']*
         italic(x)[1]['['*italic(j)*']'](italic(i))+sum()[italic(k)]*
         beta[2]['['*italic(k)*']']*italic(x)[2]['['*
         italic(k)*']'](italic(i))+sum()[italic(jk)]*
         beta[1%*%2]['['*italic(jk)*']']*
         italic(x)[1%*%2]['['*italic(jk)*']'](italic(i))") %>% 
...
p14 <-
  ...
  annotate(geom = "text",
           ...
           label = "'~'",
           size = 10, color = bd[3], family = "Times", parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
p15 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(hjust = 0, size = 7, color = bd[3], parse = TRUE, 
            family = "Times") +
...
p16 <-
  ...
  annotate(geom = "text",
           ...
           label = c("'~'", "nu*minute+1", "'='"),
           size = c(10, 7, 10), color = bd[3], family = "Times", parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
p18 <-
  ...
  annotate(geom = "text",
           x = c(0.56, 0.7), y = 0.5,
           label = c("'~'", "italic(jk)"),
           size = c(10, 7), color = bd[3], family = "Times", parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
**Warning**: this one took several hours fit.
...
fit20.2 <-
  brm(data = my_data,
      ...
      control = list(adapt_delta = 0.9995,
                     max_treedepth = 15),
      stanvars = stanvars,
      file = "~/Desktop/DBDA/fits/fit20.02")
``` 
...
as_draws_df(fit20.2) %>% 
  ...
  levels = c("Normality", "Mean of Cell sigma's", "SD of Cell sigma's"))) %>% 
...
f <-
  ...
  transmute(`Full - Assis @ CHEM` = Full_CHEM - Assis_CHEM,
            `Full - Assis @ PSY`  = Full_PSY - Assis_PSY) %>% 
  mutate(`Full.v.Assis\n(x)\nCHEM.v.PSY` = 
                 `Full - Assis @ CHEM` - `Full - Assis @ PSY`) %>% 
  pivot_longer(everything()) %>% 
  mutate(name = factor(name, levels = c("Full - Assis @ PSY", 
        "Full - Assis @ CHEM", "Full.v.Assis\n(x)\nCHEM.v.PSY")))
...
"Which model is a better description of the data? $\ldots$ In principle, an intrepid programmer could do a Bayesian model comparison$\ldots$" (pp. 605--606). We could also examine information criteria, like the LOO:
...
Here's the four-subject response time data:
...
```{r, warning = FALSE, message = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/SplitPlotAgriData.csv")
...
fit20.3 <-
  brm(data = my_data,
      ...
      control = list(adapt_delta = 0.99,
                     max_treedepth = 12),
      stanvars = stanvars,
      file = "~/Desktop/DBDA/fits/fit20.03")
``` 
...
```{r, fig.width = 8, fig.height = 2.75, warning = FALSE}
as_draws_df(fit20.3) %>% 
...
my_data %>% 
  distinct(Till, Fert) %>% 
  add_epred_draws(fit20.3, ndraws = 20,
  ...
  geom_path(data = my_data,
            ...
            size = 1/4, color = bd[5]) +
  geom_ridgeline(aes(height = -density, 
        group = interaction(Fert, .draw), color = Fert),
        fill = NA, size = 1/3, scale = 3/4,
        min_height = NA) +
  geom_jitter(data = my_data,
              height = 0.025, alpha = 1/2, color = bd[5]) +
...
`Moldbrd\nvs\nRidge` = ((Moldbrd_Broad + Moldbrd_Deep + Moldbrd_Surface) / 3) - 
  ((Ridge_Broad + Ridge_Deep + Ridge_Surface) / 3),
            
    `Moldbrd.Ridge\nvs\nChisel` = ((Moldbrd_Broad + Moldbrd_Deep + 
      Moldbrd_Surface + Ridge_Broad + Ridge_Deep + Ridge_Surface) / 6) - 
  ((Chisel_Broad + Chisel_Deep + Chisel_Surface) / 3),
    
    `Deep.Surface\nvs\nBroad` = ((Chisel_Deep + Moldbrd_Deep + Ridge_Deep + 
      Chisel_Surface + Moldbrd_Surface + Ridge_Surface) / 6) - 
  ((Chisel_Broad + Moldbrd_Broad + Ridge_Broad) / 3),
    
`Chisel.Moldbrd.v.Ridge\n(x)\nBroad.v.Deep.Surface` = 
        (((Chisel_Broad + Chisel_Deep + Chisel_Surface + Moldbrd_Broad + 
             Moldbrd_Deep + Moldbrd_Surface) / 6) - 
        ((Ridge_Broad + Ridge_Deep + Ridge_Surface) / 3)) - 
          (((Chisel_Broad + Moldbrd_Broad + Ridge_Broad) / 3) - 
             ((Chisel_Deep + Moldbrd_Deep + Ridge_Deep + 
                 Chisel_Surface + Moldbrd_Surface + Ridge_Surface) / 6))) %>% 
  pivot_longer(everything()) %>% 
...
fit20.4 <-
  brm(data = my_data,
      ...
      control = list(adapt_delta = 0.999,
                     max_treedepth = 12),
      stanvars = stanvars,
      file = "~/Desktop/DBDA/fits/fit20.04")
``` 
...
my_data %>% 
  distinct(Till, Fert) %>% 
  add_epred_draws(fit20.4, ndraws = 20,
        re_formula = Yield ~ 1 + (1 | Till) + (1 | Fert) + (1 | Till:Fert),
                  dpar = c("mu", "sigma")) %>% 
  mutate(ll = qnorm(0.025, mean = mu, sd = sigma),
         ul = qnorm(0.975, mean = mu, sd = sigma)) %>% 
...
fitted(fit20.4,
  ...
  as_tibble() %>% 
  set_names(nd %>% pull(col_names)) %>% 
  transmute(`Moldbrd\nvs\nRidge` = ((Moldbrd_Broad + Moldbrd_Deep + 
    Moldbrd_Surface) / 3) - ((Ridge_Broad + Ridge_Deep + Ridge_Surface) / 3), 
    `Moldbrd.Ridge\nvs\nChisel` = ((Moldbrd_Broad + Moldbrd_Deep + 
      Moldbrd_Surface + Ridge_Broad + Ridge_Deep + Ridge_Surface) / 6) - 
      ((Chisel_Broad + Chisel_Deep + Chisel_Surface) / 3), 
    `Deep.Surface\nvs\nBroad` = ((Chisel_Deep + Moldbrd_Deep + Ridge_Deep + 
...
`Chisel.Moldbrd.v.Ridge\n(x)\nBroad.v.Deep.Surface` = 
  (((Chisel_Broad + Chisel_Deep + Chisel_Surface + Moldbrd_Broad + 
       Moldbrd_Deep + Moldbrd_Surface) / 6) - ((Ridge_Broad + Ridge_Deep + 
  Ridge_Surface) / 3)) - (((Chisel_Broad + Moldbrd_Broad + Ridge_Broad) / 3) - 
  ((Chisel_Deep + Moldbrd_Deep + Ridge_Deep + Chisel_Surface + 
      Moldbrd_Surface + Ridge_Surface) / 6))) %>% 
  pivot_longer(everything()) %>% 
  mutate(name = factor(name, levels = c("Moldbrd\nvs\nRidge", 
    "Moldbrd.Ridge\nvs\nChisel", "Deep.Surface\nvs\nBroad", 
    "Chisel.Moldbrd.v.Ridge\n(x)\nBroad.v.Deep.Surface"))) %>% 
...
```{r, echo = FALSE}
# For some unknown reason, brms doesn't seem to let met save the results of 
...
# 
# save(lc, file = "fits/loo_compare_20.rda")
load("~/Desktop/DBDA/fits/loo_compare_20.rda")

lc %>% print(simplify = FALSE)
```
...
If you want to go that route, you're on your own.
