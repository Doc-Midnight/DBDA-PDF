...
```{r, warning = FALSE, message = FALSE, fig.width = 3, fig.height = 1}
library(lisa)
...
crossing(factory = str_c("factory ", 1:2),
  ...
  mutate(prob = c(0.75, 0.25, 0.25, 0.75)) %>% 
...
```{r, fig.width = 3.9, fig.height = 5, message = FALSE}
library(patchwork)
library(ggforce)
...
p2 <-
  ...
  annotate(geom = "text",
           x = 0.375, y = 1/3,
           label = "'~'",
           size = 10, color = kh[1], family = "Times", parse = TRUE) +
  xlim(0, 1) +
  theme_void()
...
p5 <-
  tibble(x = 0:1,
         d = (dbinom(x, size = 1, prob = 1/3)) / max(dbinom(x, size = 1, 
        prob = 1/3))) %>% 
  
  ggplot(aes(x = x, y = d)) +
  geom_col(fill = alpha(kh[4], 0.85), width = 0.4) +
  annotate(geom = "text",
           x = 0.5, y = 0.2,
           label = "Bernoulli",
           size = 7, color = kh[1]) +
  annotate(geom = "text",
           x = 0.5, y = 0.94,
           label = "theta", 
           size = 7, color = kh[1], family = "Times", parse = TRUE) +
  xlim(-0.75, 1.75) +
  theme_void() +
  theme(axis.line.x = element_line(size = 0.5, color = kh[1]))
...
p6 <-
  tibble(x = 0:1,
         d = (dbinom(x, size = 1, prob = 2/3)) / max(dbinom(x, size = 1, 
        prob = 2/3))) %>% 
...
p7 <-
  ...
  geom_text(size = c(10, 7), parse = TRUE, family = "Times") +
...
p8 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = 7, color = kh[1], parse = TRUE, family = "Times") +
  xlim(0, 2) +
  theme_void()
...
Here we rehearse if we have $\operatorname{beta}(\theta, a, b)$ prior for $\theta$ of the Bernoulli likelihood function, then the analytic solution for the posterior is $\operatorname{beta} \left( \theta \mid z + a, N - z + b \right)$. Within this paradigm, if you would like to compute $p \left( D \mid m \right)$, don't use the following function. If suffers from [underflow](https://en.wikipedia.org/wiki/Arithmetic_underflow) with large values.
...
You'd use it like this to compute $p \left( D \mid m_1 \right)$.

```{r}
p_d(z = 6, n = 9, a = 3.5, b = 8.5)
```

So to compute our BF, $\frac{p \left( D \mid m_1 \right)}{p \left( D \mid m_2 \right)}$, you might use the `p_d()` function like this.
...
Since the BF itself is only $\text{BF} = \frac{p \left( D \mid m = 1 \right)}{p \left( D \mid m = 2 \right)}$, we'd need to bring in the priors for the models themselves to get the posterior probabilities, which follows the form

$$\frac{p \left( m = 1 \mid D \right)}{p \left( m = 2 \mid D \right)} = \left[ \frac{p \left( D \mid m = 1 \right)}{p \left( D \mid m = 2 \right)} \right] \left[ \frac{p ( m = 1 )}{p(m = 2)} \right].$$

If for both our models $p(m) = 0.5$, then the BF is the same it was, before.

```{r}
(p_d_1 * .5) / (p_d_2 * .5)
```

As Kruschke pointed out, because we're working in the probability metric, the sum of $p \left( m = 1 \mid D \right)$ and $p \left( m = 2 \mid D \right)$ must be 1. By simple algebra then,

$$p \left( m = 2 \mid D \right) = 1 - p \left( m = 1 \mid D \right).$$

Therefore, it's also the case that

$$\frac{p \left( m = 1 \mid D \right)}{1 - p \left( m = 1 \mid D \right)} = 0.2135266.$$
...
$$p \left( m = 1 \mid D \right) = \frac{0.2135266}{1 + 0.2135266} = 0.1759554.$$
...
Given the data, the two models and the prior assumption they were equally credible, we conclude $m = 2$ is 0.82 probable.
...
```{r, fig.width = 3.5, fig.height = 3, warning = FALSE, message = FALSE}
p13 <-
  tibble(omega = seq(from = 0, to = 1, length.out = length)) %>% 
  mutate(m_p = ifelse(omega %in% c(0.25, 0.75), 15, 0)) %>% 
  
  ggplot(aes(xmin = 0, xmax = m_p, y = omega)) +
  geom_ribbon(fill = kh[4], color = kh[4]) +
  scale_x_continuous(expand = expansion(mult = c(0.002, 0.05)), 
        limits = c(0, 25)) +
...
d <-
  crossing(omega = seq(from = 0, to = 1, length.out = length),
           theta = seq(from = 0, to = 1, length.out = length)) %>% 
  mutate(prior = ifelse(omega == 0.25, dbeta(theta, 3.5, 8.5),
...
p23 <-
  ...
  mutate(omega = factor(str_c("omega == ", omega),
                        levels = str_c("omega == ", c(0.75, 0.25)))) %>%
...
```{r, fig.width = 6.5, fig.height = 9, warning = FALSE, message = FALSE}
p21 <-
  tibble(x = 1,
         y = 8:7,
         label = c("Prior", "K==12"),
  ...
  geom_text(aes(size = size), 
            color = kh[1], parse = TRUE, hjust = 0, show.legend = F) +
...
p33 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(aes(size = size), 
            hjust = 0, show.legend = FALSE, color = kh[1]) +
  scale_size_continuous(range = c(3.5, 5.5)) +
  coord_cartesian(xlim = c(1, 2),
                  ylim = c(4, 11)) +
  theme(axis.text = element_text(color = kh[9]),
...
# combine and plot!
(p11 / p21 / p11 / p11 / p51) | (p12 / p22 / p32 / p42 / p52) | 
  (p13 / p23 / p33 / p43 / p53)
```
...
$$\frac{p \left( m = 1 \mid D \right)}{p \left( m = 2 \mid D \right)} = 0.213,$$
...
$$\frac{p \left( m = 2 \mid D \right)}{p \left( m = 1 \mid D \right)},$$
...
But anyway, both the posterior distributions in the figure and the BF indicate $\omega = 0.75$ is a better representation of the data than $\omega = 0.25$.
...
Now we have our `stanvars` object, we are ready to fit the first model (i.e., the model for which $\omega = 0.75$).
...
fit10.1 <-
  brm(data = trial_data, 
      ...
      file = "~/Desktop/DBDA/fits/fit10.01")
```
...
That matches up nicely with Kruschke's value at the top of page 278! Let's rinse, wash, and repeat for $\omega = 0.25$. First, we'll need to redefine `omega` and our `stanvars`.
...
fit10.2 <-
  brm(data = trial_data, 
      ...
      stanvars = stanvars,
      control = list(adapt_delta = 0.999),
      file = "~/Desktop/DBDA/fits/fit10.02")
```
...
This calculation is not Available in brms; we'll cover information criteria instead.

I'm not aware of a way to specify a model "in which the top-level parameter is the index across models" in **brms** (p. 278). If you know of a way, please [share your code](https://github.com/ASKurz/Doing-Bayesian-Data-Analysis-in-brms-and-the-tidyverse/issues).
...
```{r ic_fit10.1_fit10.2, message = FALSE}
...
color_scheme_set(scheme = c(lisa_palette("KatsushikaHokusai", n = 9, 
        type = "continuous")[6:1]))
...
fit10.3 <-
  brm(data = d,
      ...
      chains = 4, cores = 4,
      seed = 10,
      file = "~/Desktop/DBDA/fits/fit10.03")
...
fit10.4 <-
  brm(data = d,
      ...
      chains = 4, cores = 4,
      seed = 10,
      file = "~/Desktop/DBDA/fits/fit10.04")
```
...
tibble(x = seq(from = 1, to = 100, by = 1)) %>% 
  ...
  scale_y_continuous(NULL, breaks = NULL,
                     expand = expansion(mult = c(0, 0.05))) +
  scale_x_continuous(expression(italic(p)(nu)), 
        expand = expansion(mult = c(0, 0.01)), limits = c(0, 100))
```
...
```{r ic_fit10.3_and_fit10.4, warning = FALSE, message = FALSE}
...
>
> $$p_b \left( \hat y \mid D, m = b \right) = \int {\rm d} \theta_b \; p_b \left( \hat y \mid \theta_b, m = b \right) p_b \left( \theta_b \mid D, m = b \right)$$
>
...
>
> \begin{align*}
> p \left( \hat y \mid D \right) & = \sum_m p \left( \hat y \mid D, m \right) p \left( m \mid D \right) \\
> & = \sum_m \int {\rm d} \theta_m \; p_m \left( \hat y \mid \theta_m, m \right) p_m \left( \theta_m \mid D, m \right) p \left( m \mid D \right)
> \end{align*}
>
...
> recall that there were two models of mints that created the coin, with one mint being tail-biased with mode $\omega = 0.25$ and one mint being head-biased with mode $\omega = 0.75$ The two subpanels in the lower-right [of Figure 10.3] illustrate the posterior distributions on $\omega$ within each model, $p \left( \theta \mid D, \omega = 0.25 \right)$ and $p \left( \theta \mid D, \omega = 0.75 \right)$ The winning model was $\omega = 0.75$, and therefore the predicted value of future data, based on the winning model alone, would use $p \left( \theta \mid D, \omega = 0.75 \right)$. (p. 289)

Here's the histogram for $p \left( \theta \mid D, \omega = 0.75 \right)$, which we generate from our `fit10.1`.
...
as_draws_df(fit10.1) %>% 
  ...
  stat_histinterval(point_interval = mode_hdi, .width = c(0.95, 0.5),
                    ...
                    breaks = 40, slab_size = 0.25, outline_bars = TRUE) +
  scale_x_continuous(expression(italic(p)(theta*"|"*italic(D)*", 
        "*omega==0.75)), expand = expansion(mult = c(0, 0)), 
...
> But the overall model included $\omega = 0.75$, and if we use the overall model, then the predicted value of future data should be based on the complete posterior summed across values of $\omega$. The complete posterior distribution [is] $p \left( \theta \mid D \right)$ (p. 289).
...
```{r pp_averaged_fit10.1_fit10.2, warning = FALSE, message = FALSE}
nd <- tibble(y = 1)
...
As Kruschke concluded, "you can see the contribution of $p \left( \theta \mid D, \omega = 0.25 \right)$ as the extended leftward tail" (p. 289). Interestingly enough, that looks a lot like the marginal density of $p \left( \theta \mid D \right)$ across values of $\omega$ we made with grid approximation in Figure 10.3, doesn't it?

## Model Complexity Naturally Taken into Account
...
* $p \left( \theta \mid D, \kappa = 2000 \right)$ (i.e., the "must-be-fair" model) and
* $p \left( \theta \mid D, \kappa = 2 \right)$ (i.e., the "anything's-possible" model).
...
tibble(omega = 0.5,
  ...
  scale_x_continuous(expression(theta), expand = expansion(mult = c(0, 0)), 
                     breaks = 0:5 / 5, labels = c("0", "0.2", "0.4", "0.6", 
                        "0.8", "1"),
                     limits = c(0, 1)) +
  scale_y_continuous(NULL, breaks = NULL,
                     expand = expansion(mult = c(0, 0.05))) +
...
```{r, warning = FALSE, message = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/BattingAverage.csv")
...
fit9.2 <-
  brm(data = my_data,
      ...
      seed = 9,
      file = "~/Desktop/DBDA/fits/fit09.02")
```
...
fit10.5 <-
  update(fit9.2,
         newdata = my_data,
         formula = Hits | trials(AtBats) ~ 1 + (1 | PriPos_small) + 
                 (1 | PriPos_small:Player),
         iter = 3500, warmup = 500, chains = 3, cores = 3,
         control = list(adapt_delta = 0.99),
         seed = 10,
         file = "~/Desktop/DBDA/fits/fit10.05")
```

Unlike with what Kruschke alluded to in the prose, here we'll compare the two models with the WAIC.

```{r waic_fit9.2_fit10.5, warning = FALSE, message = FALSE}
...
```{r}
p_d(z, n, a = 500, b = 500) / p_d(z, n, a = 0.01, b = 0.01)
```
...
tibble(alpha = c(1, 0.01),
       beta  = c(1, 0.01),
       model = factor(c("Uninformative prior, beta(1, 1)", 
                        "Haldane prior, beta(0.01, 0.01)"),
                      levels = c("Uninformative prior, beta(1, 1)", 
                                 "Haldane prior, beta(0.01, 0.01)"))) %>%
...
Recall that when we combine a $\operatorname{beta} \left( \theta \mid \alpha, \beta \right)$ prior with the results of a Bernoulli likelihood, we get a posterior defined by $\operatorname{beta} \left( \theta \mid z + \alpha, N - z + \beta \right)$.
...
d %>% 
  ...
  scale_x_continuous(expression(theta), expand = expansion(mult = c(0, 0)), 
                     breaks = 0:5 / 5, labels = c("0", "0.2", "0.4", 
                        "0.6", "0.8", "1"),
                     limits = c(0, 1)) +
...
# Peaked vs Uniform
p_d(z, n, a = 500 + 6, b = 500 + 10 - 6) / p_d(z, n, 
        a =   1 + 6, b =   1 + 10 - 6)

# Peaked vs Haldane
p_d(z, n, a = 500 + 6, b = 500 + 10 - 6) / p_d(z, n, a = 0.01 + 6, 
        b = 0.01 + 10 - 6)
```
...
fit10.6 <-
  brm(data = trial_data, 
      ...
      iter = 11000, warmup = 1000, chains = 4, cores = 4,
      seed = 10,
      file = "~/Desktop/DBDA/fits/fit10.06")
...
fit10.7 <-
  brm(data = trial_data, 
      ...
      iter = 11000, warmup = 1000, chains = 4, cores = 4,
      seed = 10,
      file = "~/Desktop/DBDA/fits/fit10.07")
...
fit10.8 <-
  brm(data = trial_data, 
      ...
      iter = 11000, warmup = 1000, chains = 4, cores = 4,
      seed = 10,
      file = "~/Desktop/DBDA/fits/fit10.08")
```

Compare the models by the LOO.

```{r loo_fit10.6_through_fit10.6, message = FALSE}
...
```{r mw_fit10.6_through_fit10.8, cache = TRUE}
mw <-
...
p1 <-
  pp_check(fit10.6, type = "bars", ndraws = 1000) +
  ggtitle("fit10.6",
          subtitle = expression("beta"*(500*", "*500)))

p2 <-
  pp_check(fit10.7, type = "bars", ndraws = 1000) +
  ggtitle("fit10.7",
          subtitle = expression("beta"*(1*", "*1)))

p3 <-
  pp_check(fit10.8, type = "bars", ndraws = 1000) +
  ggtitle("fit10.8",
