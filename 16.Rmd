...
$$p \left(y \mid \mu, \sigma \right) = \frac{1}{\sigma \sqrt{2 \pi}} \exp \left[ -\frac{1}{2} \frac{ \left(y - \mu \right)^2}{\sigma^2} \right],$$
...
d <-
  ...
  mutate(density = dnorm(y, mean = mu, sd = sigma),
         ...
         sigma   = factor(sigma, labels = str_c("sigma==", 
        c(7.35, 12.2, 18.4))))
...
```{r, message = FALSE}
library(readr)
my_data <- read_csv("~/Desktop/DBDA/data.R/TwoGroupIQ.csv")
...
tibble(x = seq(from = 0, to = 12, by = 0.025)) %>% 
  ...
  labs(subtitle = "The grey density in the background is the conventional 
 gamma prior for precision. The dashed vertical line is our precision value.") +
...
tibble(x   = seq(from = 0, to = 110, by = 0.1)) %>% 
  ...
  scale_x_continuous(expand = expansion(mult = c(0, 0.05))) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), breaks = NULL) +
  labs(subtitle = "The gray density in the background is the half-normal prior 
       for sigma. The dashed vertical line is our 'sd_y' value.") +
...
tibble(x = 0:(sd_y * 1000)) %>% 
  ...
  scale_x_continuous(expand = expansion(mult = c(0, 0.05))) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), breaks = NULL) +
  labs(subtitle = "The gray density in the background is Kruschke's uniform 
 prior for sigma. The dashed vertical line is our 'sd_y' value.") +
  coord_cartesian()
```
...
tibble(x = seq(from = -10000, to = 10000, by = 1)) %>% 
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), breaks = NULL) +
  labs(subtitle = "The grey density in the background is the normal prior for 
 mu. The dashed vertical line is our 'mean_y' value.")
```
...
```{r, fig.width = 3.9, fig.height = 3.5, message = FALSE}
library(patchwork)
...
p1 <-
  ...
  annotate(geom = "text",
           ...
           size = 7, color = bp[5], family = "Times", parse = TRUE) +
...
p3 <-
  ...
  annotate(geom = "text",
           x = c(0.3, 1), y = 0.6,
           label = "'~'",
           size = 10, color = bp[5], family = "Times", parse = TRUE) +
  xlim(0, 2) +
  theme_void()
...
p5 <-
  ...
  geom_text(size = c(10, 7), color = bp[5], parse = TRUE, family = "Times") +
...
p6 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = 7, color = bp[5], parse = TRUE, family = "Times") +
  xlim(0, 1) +
  theme_void()
...
fit16.1 <-
  brm(data = my_data,
      ...
      stanvars = stanvars, 
      seed = 16,
      file = "~/Desktop/DBDA/fits/fit16.01")
```
...
```{r, fig.width = 3, fig.height = 2.25, warning = FALSE, message = FALSE}
draws %>% 
  ggplot(aes(x = Score)) + 
  geom_histogram(data = my_data, 
                 aes(y = stat(density)),
                 fill = bp[2],
                 size = 0.2, binwidth = 5, boundary = 0) +
...
```{r, fig.width = 4, fig.height = 2.25, warning = FALSE, message = FALSE}
library(bayesplot)
...
```{r, fig.width = 6, fig.height = 3}
tibble(y = seq(from = -10, to = 20, length.out = 1000)) %>% 
  ggplot(aes(x = y)) +
...
# simulate the data
d <- tibble(y = metRology::rt.scaled(n = 177, df = 2.63, 
        mean = 1.11, sd = 0.15))

# plot
tibble(y = seq(from = -3, to = 12, length.out = 1000)) %>% 
  ggplot(aes(y)) +
  ...
  geom_line(aes(y = dnorm(y, mean = 1.16, sd = 0.63)),
            color = bp[2]) +
  geom_line(aes(y = metRology::dt.scaled(y, df = 2.63, mean = 1.11, 
        sd = 0.15)),
        color = bp[2]) +
...
```{r, fig.width = 2, fig.height = 8, eval = FALSE}
...
# the primary data
d <-
  crossing(y  = seq(from = -8, to = 8, length.out = 1000),
           nu = c(Inf, 5, 2, 1)) %>%
...
d <-
  ...
  expand(nesting(nu, ymin, ymax),
         y = seq(from = -8, to = 8, length.out = 1000)) %>%
...
fit16.2 <-
  brm(data = my_data,
      ...
      stanvars = stanvars,
      seed = 16,
      file = "~/Desktop/DBDA/fits/fit16.02")
```
...
p1 <-
  ...
  geom_histogram(fill = bp[2],
                 size = 0.2, binwidth = 5, boundary = 1) +
  stat_pointinterval(aes(y = 0), 
                     point_interval = mode_hdi, .width = 0.5, color = bp[6]) +
  scale_y_continuous(NULL, breaks = NULL) +
...
```{r, fig.width = 2.5, fig.height = 4, warning = FALSE}
...
draws %>% 
  ...
  expand(nesting(.draw, b_Intercept, sigma, nu),
         Score = seq(from = 40, to = 250, by = 1)) %>% 
  mutate(density = metRology::dt.scaled(x = Score, df = nu, mean = b_Intercept, 
    sd = sigma)) %>% 
...
```{r, fig.width = 6, fig.height = 1.25, warning = FALSE}
...
Note the last line in the parameters block:

`real\<lower=1\> nu;  // degrees of freedom or shape.` 

By default, **brms** set the lower bound for $\nu$ to 1.
...
```{r, fig.width = 5.8, fig.height = 3.5, message = FALSE}
# exponential density
p1 <-
...
p4 <-
  ...
  geom_segment(arrow = my_arrow, color = bp[4]) +
  annotate(geom = "text",
           x = c(0.3, 0.65, 1.38, 1.62, 2, 2.3), 
           y = c(0.92, 0.25, 0.6, 0.6, 0.6, 0.6),
...
fit16.3 <-
  brm(data = my_data,
      ...
      stanvars = stanvars,
      seed = 16,
      file = "~/Desktop/DBDA/fits/fit16.03")
```
...
draws <-
  ...
  mutate(`Difference of Means`  = `Smart Drug Mean` - `Placebo Mean`,
         ...
         `Effect Size` = (`Smart Drug Mean` - `Placebo Mean`) / 
           sqrt((`Smart Drug Scale`^2 + `Placebo Scale`^2) / 2)) %>% 
...
levels <- c("Placebo Mean", "Smart Drug Mean",  "Placebo Scale", 
            "Difference of Means", "Smart Drug Scale", "Difference of Scales", 
            "Normality", "Effect Size")

# we'll use this to mark off the ROPEs as white strips in the background
rope <-
  tibble(name = factor(c("Difference of Means", "Difference of Scales", 
                         "Effect Size"),
                       levels = levels), 
         xmin = c(-1, -1, -0.1),
         xmax = c(1, 1, 0.1))
...
draws %>% 
  tibble() %>% 
  slice_sample(n = n_lines) %>% 
  expand(nesting(.draw, `Placebo Mean`, `Smart Drug Mean`, 
                 `Placebo Scale`, `Smart Drug Scale`, Normality),
         Score = seq(from = 40, to = 250, by = 1)) %>% 
  
  mutate(Placebo      = metRology::dt.scaled(x = Score, 
        df = 10^Normality, mean = `Placebo Mean`,    sd = `Placebo Scale`),
        `Smart Drug` = metRology::dt.scaled(x = Score, 
        df = 10^Normality, mean = `Smart Drug Mean`, 
        sd = `Smart Drug Scale`)) %>% 
...
$$\sigma_p^* = \sqrt{\frac{ \left( n_1 - 1 \right)\sigma_1^2 + \left(n_2 - 1 \right)\sigma_2^2}{n_1 + n_2 - 2}},$$
...
$$d = \frac{\mu_2 - \mu_1}{\sqrt{ \left(\sigma_1^2 + \sigma_2^2 \right) / 2}},$$

when $n_1 = n_2$, or

$$d = \frac{\mu_2 - \mu_1}{\sqrt{ \left[ \left(n_1 - 1 \right)\sigma_1^2 + \left(n_2 - 1 \right)\sigma_2^2 \right] / \left(n_1 + n_2 - 2 \right)}},$$
