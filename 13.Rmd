...
2. increasing the effect size, and
...
> Usually it is more intuitively accessible to get prior data, or to think of idealized prior data, than to directly specify a distribution over parameter values. For example, based on knowledge about the application domain, we might have 2,000 actual or idealized flips of the coin for which the result showed 65% heads. Therefore we'll describe the data-generating hypothesis as a beta distribution with a mode of 0.65 and concentration based on 2,000 flips after a uniform "proto-prior": $\operatorname{beta} \left( \theta \mid 0.65 \cdot (2000 - 2) + 1, \; (1 - 0.65) \cdot (2000 - 2) + 1 \right)$ (p. 366).
...
Here's what $\operatorname{beta} \left( \theta \mid 0.65 \cdot (2{,}000 - 2) + 1, (1 - 0.65) \cdot (2{,}000 - 2) + 1 \right)$ looks like.

```{r, fig.width = 4, fig.height = 2, message = FALSE, warning = FALSE}
kappa <- 2000
omega <- 0.65
...
tibble(theta = seq(from = 0, to = 1, by = 0.001)) %>% 
  ...
  geom_area(fill = ce[3]) +
  scale_y_continuous(NULL, breaks = NULL, expand = 
                             expansion(mult = c(0, 0.05))) +
  labs(subtitle = 
        expression("The beta"*(1299.7*', '*700.3)*" prior."),
       x = expression(theta))
```
...
And indeed our samples did tend around $\theta = 0.643$. Had we increased our number of draws by an order of magnitude or two, our proportion of heads would have been even closer to the true data-generating value.
  ...
  function(gen_prior_mode, gen_prior_n,
    ...
    while(not_powerful_enough) {
      ...
      if (!is.null(null_value)) {
        power_hdi <- sum(p_z_vec[hdi_matrix[, 1] > rope[2] | 
                hdi_matrix[, 2] < rope[1]])
      }
      if (verbose) {
...
```{r sim, results = "hide"}
sim <-
  crossing(mode  = seq(from = 0.6, to = 0.85, by = 0.05),
           power = c(0.7, 0.8, 0.9)) %>% 
...
Recall that if we have $\operatorname{beta}(a, b)$ prior for $\theta$ of the Bernoulli likelihood function, then the analytic solution for the posterior is $\operatorname{beta} \left(\theta \mid z + a, \; N – z + b \right)$. In our first example, $z = 6$ out of $N = 10$ randomly selected voters preferred candidate A and we started with a flat $\operatorname{beta} \left( \theta \mid 1, 1 \right)$ prior. We can check that our posterior is indeed $\operatorname{beta}(7, 5)$ by working through the algebra.
...
If you look back up at our `min_n_for_hdi_power()` defining code, above, you'll see that "One and only one of `hdi_max_width` and `null_value` must be specified." So if we want to determine the necessary $N$ for an 95% HDI width of less than 0.2, we need to set `hdi_max_width = 0.2` and `null_value = NULL`.

```{r}
min_n_for_hdi_power(gen_prior_mode = 0.75, 
                    gen_prior_n    = 10,
                    hdi_max_width  = 0.2,  # Set value here
                    null_value     = NULL, 
                    rope           = NULL,
                    desired_power  = 0.8,
                    aud_prior_mode = 0.5, 
...
sim %>% 
  ...
  geom_area(fill = ce[3]) +
  scale_x_continuous(expression(theta), labels = c("0", "", "0.5", "", "1")) +
  scale_y_continuous(NULL, breaks = NULL, expand = expansion(mult = c(0, 0.05))) +
...
fit13.1 <-
  brm(data = tibble(y = rbinom(n, size, omega)),
      ...
      file = "~/Desktop/DBDA/fits/fit13.01")
```
...
Looks like a dream. Let's move forward and run the simulation proper. In his script file, Kruschke suggested we simulate with large $N$s like 1,000 or so. Since this is just an example, I'm WILL cut that to 100 here:
...
```{r, echo = FALSE}
# Even after reducing `sim1` through `sim5` by an order of magnitude, they 
# take a long time to complete. So instead of completing them on the fly 
# for each new version of this project, I've saved the results of `sim1` 
# through `sim5` in an external file. 
# save(list = c("sim1", "sim2", "sim3", "sim4", "sim5"), file = "13_sims/power_sims.rda")
load("~/Desktop/DBDA/13_sims/power_sims.rda")
...
Here are the idealized parameters Kruschke outlined on pages 376--377.
...
d <-
  ...
  mutate(theta_transformed = ((theta - mean(theta)) / 
    sd(theta)) * ideal_group_sd + ideal_group_mean) %>% 
...
fit13.2 <-
  brm(data = d,
      ...
      seed = 13,
      file = "~/Desktop/DBDA/fits/fit13.02")
```
...
fit13.3 <-
  brm(data = sample_theta(seed = 13, n_subj = 14) %>% 
      ...
      seed = 13,
      control = list(adapt_delta = 0.9),
      file = "~/Desktop/DBDA/fits/fit13.03")
```
...
```{r get_theta_s_hdi, warning = FALSE, message = FALSE}
...
sim3 <-
  ...
  group_by(seed) %>% 
  mutate(pass_rope_theta_s  = sum(exceed_rope_theta_s) > 0 & 
           sum(below_rope_theta_s) == 0,
...
d <-
  tibble(s     = 1:ideal_n_subj,
         theta = rbeta(ideal_n_subj, b$a, b$b)) %>% 
  mutate(theta_transformed = ((theta - mean(theta)) / sd(theta)) * 
           ideal_group_sd + ideal_group_mean) %>% 
  mutate(theta_transformed = ifelse(theta_transformed >= 0.999, 0.999,
                                    ifelse(theta_transformed <= 0.001, 0.001,
                                           theta_transformed))) %>% 
  mutate(z = round(theta_transformed * ideal_n_trl_per_subj)) %>% 
...
fit13.4 <-
  update(fit13.2,
         ...
         cores = 4,
         seed = 13,
         file = "~/Desktop/DBDA/fits/fit13.04")
```
...
>
> Unfortunately, that intuition about independence across flips only tells part of story. What's missing is the realization that the stopping procedure biases which data are sampled, because the procedure stops only when extreme values happen to be randomly sampled$\ldots$
>
...
To start our sequence of simulated coin flips, we'll set the total number of trials we'd like, `n_trial`, specify our `bias`, and set out seed. Like Kruschke did in the text we'll do `bias <- 0.5`, first.
...
$$
\frac{p \left( z, N \mid M_\text{alt} \right)}{p \left( z, N \mid M_\text{null} \right)} = \frac{B \left( z + a_\text{alt}, \; N - z + b_\text{alt} \right) / B \left( a_\text{alt}, \; b_\text{alt} \right)}{\theta_\text{null}^z \left( 1 - \theta_\text{null} \right)^{(N - z)}}.
$$

To ground ourselves a bit, here's some of the content from the page that followed the equation:

> For a default alternative prior, the beta distribution is supposed to be uninformed, according to particular mathematical criteria. Intuition might suggest that a uniform distribution suits this requirement, that is, $\operatorname{beta} \left( \theta \mid 1, 1 \right)$. Instead, some argue that the most appropriate uninformed beta distribution is $\operatorname{beta} \left( \theta \mid \epsilon, \epsilon \right)$, where $\epsilon$ is a small number approaching zero (p. 344).
...
p2 <-
  ...
  scale_x_continuous(NULL, breaks = 0:7 * 100) +
  scale_y_continuous(expression(italic(p)*"-value"), expand = 
        expansion(mult = 0), limits = c(0, 1))
...
p4 <-
  ...
  scale_color_manual(values = ce[c(2, 7)], breaks = NULL) +
  scale_x_continuous(NULL, breaks = 0:7 * 100) +
  scale_y_continuous("95% HDI", expand = expansion(mult = 0), 
                     limits = c(0, 1))
...
p5 <-
  ...
  scale_color_manual(values = ce[c(2, 7)], breaks = NULL) +
  scale_x_continuous(NULL, breaks = 0:7 * 100) +
  scale_y_continuous("HDI width", expand = expansion(mult = 0), 
                     limits = c(0, 1))
```
...
coin.65 <-
  ...
  mutate(p = map2_dbl(n, z, fit_glm)) %>% 
  # log(BF)
  mutate(log_bf = map2_dbl(n, z, log_bf, theta = 0.5)) %>%
  # HDIs
  mutate(hdi = map2(n, z, ~hdi_of_qbeta(.y + 1, .x - .y + 1))) %>% 
  unnest(hdi) %>% 
  # HDI width
...
p3 <-
  ...
  annotate(geom = "text", x = 60, y = c(-8, 28), 
           label = c("accept the null", "reject the null"), 
           color = ce[1]) +
  scale_color_manual(values = ce[c(2, 7)], breaks = NULL) +
...
> In NHST, the overall $p$-value for any particular test is increased when the test is considered in the space of all other intended tests$\ldots$
>
