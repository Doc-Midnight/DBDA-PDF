...
\begin{align*}
y_i & \sim \operatorname{Normal}(\mu, \sigma = 2), \text{in which} \\
\mu & = 10 + 2 x_i.
\end{align*}
...
curves <-
  ...
  # based on a Gaussian with `mean = y_mean` and `sd = 2`, 
  # compute the 99% intervals
  mutate(ll = qnorm(0.005, mean = y_mean, sd = 2),
         ul = qnorm(0.995, mean = y_mean, sd = 2)) %>%
...
d %>% 
  ...
  stat_smooth(method = "lm", se = FALSE, fullrange = TRUE, color = bp[1]) +
  geom_path(data = curves,
            aes(group = y_mean),
            color = bp[2], size = 1) +
  labs(title = "Normal PDF around Linear Function",
       subtitle = "We simulated x from a uniform distribution in the left 
 panel and simulated it from a mixture of\n two Gaussians on the right.") +
...
p1 <-
  ...
  annotate(geom = "text",
           ...
           size = 7, color = bp[1], family = "Times", parse = TRUE) +
...
p3 <-
  ...
  annotate(geom = "text",
           x = c(0.4, 1.25), y = 0.5,
           label = "'~'",
           size = 10, color = bp[1], family = "Times", parse = TRUE) +
  xlim(0, 2) +
  theme_void()
...
p5 <-
  ...
  geom_text(size = 7, color = bp[1], parse = TRUE, family = "Times") +
...
p9 <-
  ...
  ggplot(aes(x = x, y = y, label = label)) +
  geom_text(size = c(10, 7), color = bp[1], parse = TRUE, family = "Times") +
...
HtWtDataGenerator <- function(nSubj, rndsd = NULL, maleProb = 0.50) {
  ...
  for (i in 1:nSubj)  {
    ...
    if (sex == maleval) {datum = MASS::mvrnorm(n = 1, 
        mu = Mmean, Sigma = Msigma)}
    if (sex == femaleval) {
      Fclust = sample(c(1, 2), size = 1, replace = TRUE, 
        prob = c(prop1, prop2))
      if (Fclust == 1) {datum = MASS::mvrnorm(n = 1, 
        mu = Fmean1, Sigma = Fsigma1)}
      if (Fclust == 2) {datum = MASS::mvrnorm(n = 1, 
        mu = Fmean2, Sigma = Fsigma2)}
    }
...
fit17.1 <-
  brm(data = d,
      ...
      stanvars = stanvar(1/29, name = "one_over_twentynine"),
      seed = 17,
      file = "~/Desktop/DBDA/fits/fit17.01")
...
fit17.2 <-
  update(fit17.1,
         ...
         chains = 4, cores = 4,
         seed = 17,
         file = "~/Desktop/DBDA/fits/fit17.02")
```
...
d %>% 
  ...
  labs(subtitle = eval(substitute(paste("Data with", 
    n_lines, "credible regression lines"))),
       x = "height",
       y = "weight") +
...
```{r, fig.width = 3.25, fig.height = 3, warning = FALSE}
draws %>% 
...
fit17.3 <-
  brm(data = d,
      ...
      stanvars = stanvars,
      seed = 17,
      file = "~/Desktop/DBDA/fits/fit17.03")
```
...
Their $N_{\rm eff}/N$ ratios were pretty similar. Both were reasonable. You'd probably want to run a simulation to contrast them with any rigor.

```{r, fig.width = 6, fig.height = 3.25, warning = FALSE, message = FALSE}
...
```{r, message = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/HierLinRegressData.csv")
...
```{r, fig.width = 8.5, fig.height = 6, message = FALSE}
# normal density
p1 <-
  tibble(x = seq(from = -3, to = 3, by = 0.1)) %>% 
  ggplot(aes(x = x, y = (dnorm(x)) / max(dnorm(x)))) +
...
fit17.4 <-
  brm(data = my_data,
      ...
      seed = 17,
      stanvars = stanvar(1/29, name = "one_over_twentynine"),
      file = "~/Desktop/DBDA/fits/fit17.04")
```
...
my_data %>% 
  ...
  geom_line(aes(group = Subj, color = Subj),
            size = 1/4) +
  scale_color_manual(values = beyonce_palette(41, n = 25, 
    type = "continuous"), breaks = NULL) +
  scale_y_continuous(breaks = seq(from = 50, to = 250, by = 50)) +
  labs(subtitle = eval(substitute(paste("Data from all units with", 
    n_lines, "credible population-level\nregression lines")))) +
...
labs(subtitle = "Each unit has its own bundle of credible regression lines") +
...
p1 <-
  ...
  scale_y_continuous(breaks = seq(from = 50, to = 250, by = 50)) +
  labs(subtitle = eval(substitute(paste("Data from all units with", 
    n_lines, "credible population-level regression lines")))) +
  coord_cartesian(xlim = c(40, 95),
                  ylim = c(30, 270)) +
...
```{r, message = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/IncomeFamszState3yr.csv",
                    comment = "#")
...
fit17.5 <-
  brm(data = my_data,
      family = student,
      median_income_z | se(se_z, sigma = TRUE) ~ 1 + family_size_z + 
              I(family_size_z^2) + 
      ...
      stanvars = stanvar(1/29, name = "one_over_twentynine"),
      seed = 17,
      file = "~/Desktop/DBDA/fits/fit17.05")
```
...
make_beta_0 <- function(zeta_0, zeta_1, zeta_2, sd_x, sd_y, m_x, m_y) {
  zeta_0 * sd_y + m_y - zeta_1 * m_x * sd_y / sd_x + zeta_2 * 
    m_x^2 * sd_y / sd_x^2}
...
my_data %>%
  ...
  geom_point(aes(color = State),
             alpha = 2/3, size = 1/2) +
  scale_color_manual(values = beyonce_palette(41, n = 52, 
        type = "continuous"), breaks = NULL) +
...
c <-
  ...
  mutate(draw  = rep(1:4000, times = 3),
         param = rep(c("Intercept", "family_size_z", "Ifamily_size_zE2"), 
                     each = 4000)) %>% 
  ...
  mutate(b_0 = make_beta_0(zeta_0 = Intercept,
         ...
         b_2 = make_beta_2(zeta_2 = Ifamily_size_zE2,
                           sd_x   = sd_x,
                           sd_y   = sd_y)) %>% 
  # we just want the first 25 states, from Alabama through Mississippi, 
  # so we'll `filter()`
...
my_data %>%
  ...
  scale_color_manual(values = beyonce_palette(41, n = 52, 
    type = "continuous"), breaks = NULL) +
  scale_x_continuous("Family size", breaks = 1:8) +
  labs(subtitle = "Each State has its own bundle of credible regression curves",
       y = "Median income") +
  coord_cartesian(xlim = c(1, 8),
                  ylim = c(0, 150000)) +
  theme(legend.position = "none") +
