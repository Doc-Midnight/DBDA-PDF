...
> Stan is the name of a software package that creates representative samples of parameter values from a posterior distribution for complex hierarchical models, analogous to JAGS$\ldots$
> 
...
To learn more about Stan from the Stan team themselves, check out the main website [https://mc-stan.org/](https://mc-stan.org/). If you like to dive deep, bookmark the [*Stan user's guide*](https://mc-stan.org/docs/2_26/stan-users-guide/index.html) [@standevelopmentteamStanUserGuide2021] and the [*Stan reference manual*](https://mc-stan.org/docs/2_26/reference-manual/) [@standevelopmentteamStanReferenceManual2021].

We won't be using Stan directly in this project. I prefer working with it indirectly through the interface of Bürkner's **brms** package instead. If you haven't already, bookmark the **brms** [GitHub repository](https://github.com/paul-buerkner/brms), [CRAN page](https://CRAN.R-project.org/package=brms), and [reference manual](https://CRAN.R-project.org/package=brms/brms.pdf) [@brms2021RM]. You can also view Bürkner's talk from the useR! International R User 2017 Conference, [*brms: Bayesian multilevel models using Stan*](https://www.youtube.com/watch?v=40o0_0XTB6E). Here's how Bürkner described **brms** in its GitHub repo:
...
fit14.1 <-
  brm(data = my_data, 
      ...
      seed = 14,
      file = "~/Desktop/DBDA/fits/fit14.01")
```

As Kruschke wrote,

> `iter` is the total number of steps per chain, including `warmup` steps in each chain. Thinning merely marks some steps as not to be used; thinning does not increase the number of steps taken. Thus, the total number of steps that Stan takes is `chains`$\cdot$`iter`. Of those steps, the ones actually used as representative have a total count of `chains`$\cdot$(`iter`-`warmup`)/`thin`. Therefore, if you know the desired total steps you want to keep, and you know the warm-up, chains, and thinning, then you can compute that the necessary `iter` equals the desired total multiplied by `thin`/`chains`+`warmup`.
>
...
fit14.2 <-
  brm(data = my_data, 
      ...
      seed = 14,
      file = "~/Desktop/DBDA/fits/fit14.02")
```
...
fit14.3 <-
  update(fit14.2,
         ...
         control = list(adapt_delta = 0.99),
         seed = 14,
         file = "~/Desktop/DBDA/fits/fit14.03")
```
...
fit14.4 <-
  brm(data = my_data, 
      ...
      sample_prior = "yes",
      seed = 14,
      file = "~/Desktop/DBDA/fits/fit14.04")
```
...
[^5]: There's another way to fix this. You may have noticed how Kruschke has a habit of modeling binomial intercepts with the identity link and with the beta distribution as the prior. This can cause algorithmic difficulties because whereas our beta prior puts lower and upper boundaries for the intercept, **brms** expects the parameter space for an intercept to be unbounded. The solution for this is to explicitly tell **brms** that you want to model the intercept with lower and upper bounds. For an example of how to do this, revisit `fit8.2` from [Section 8.4][Example: Difference of biases]. I try not to make a big deal about this in the ebook because my guess is most applied scientists won't be fitting a lot of intercepts-only binomial models.
