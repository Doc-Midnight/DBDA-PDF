...
```{r, fig.width = 6, fig.height = 1.5, warning = FALSE, message = FALSE}
tibble(xmin = 0.45,
       xmax = 0.55) %>% 
ggplot() +
  ...
  scale_x_continuous(breaks = 0:5 / 5, expand = expansion(mult = 0), 
                     limits = c(0, 1)) +
...
# recall Beta(1, 1) is flat
  mutate(prior      = dbeta(theta, shape1 = 1, shape2 = 1),          
# (i.e., p(theta))
         likelihood = bernoulli_likelihood(theta = theta,            
# (i.e., p(D | theta))
                                           data = trial_data)) %>%
  mutate(posterior = likelihood * prior / sum(prior * likelihood))   
# (i.e., p(theta | D))
...
ggplot(data = d) +
  ...
  scale_x_continuous(breaks = 0:5 / 5, expand = expansion(mult = 0), 
                     limits = c(0, 1)) +
  scale_y_continuous(NULL, breaks = NULL, expand = expansion(mult = 
        c(0, 0.05))) +
  labs(title = "Nope, that density ain't in that ROPE.",
       x = expression(theta)) 
```

With the formula by $\operatorname{beta} \left( \theta \mid z + \alpha, \; N - z + \beta \right)$, we can analytically compute the Beta parameters for the posterior.
...
c_small %>% 
  ...
  mutate(name = factor(name, levels = c("Pitcher - Catcher", 
        "Catcher - 1st Base"))) %>% 
...
# a correlation of 0.9 seems about right
correlation_matrix <- 
  matrix(c(1, 0.9, 
           0.9, 1), 
         nrow = 2, ncol = 2)
...
# now we'll use the correlation matrix and standard deviations to make a 
# covariance matrix
covariance_matrix <- 
...
To make the plots in the right panel of Figure 12.1, we just need to convert the correlation from $0.9$ to $-0.9$.

```{r}
# this time we'll make the correlations -0.9
correlation_matrix <- 
  matrix(c(1, -0.9, 
           -0.9, 1), 
         nrow = 2, ncol = 2)
...
### Why HDI and Not an Equal-Tailed Interval?
...
tibble(x = seq(from = 0, to = 40, by = 0.1)) %>% 
  ...
  ggplot(aes(x = x, y = dgamma(x, 2, 0.2))) +
  geom_area(fill = oa[4]) +
...
```{r}
(ex <- qgamma(c(0.025, 0.975), shape = 2, rate = 0.2))
```
...
(
  ey <-
    qgamma(c(0.025, 0.975), shape = 2, rate = 0.2) %>% 
...
Technically, those second and third `y`-values should be zero. I've set them a touch higher so they don't get obscured by the $x$-axis in the plot. In any case, we're finally ready to plot a more complete version of Figure 12.2.
...
d <-
  ...
  mutate(prior      = dbeta(x = theta, shape1 = epsilon, shape2 = epsilon),
         likelihood = bernoulli_likelihood(theta = theta,
                                           data = trial_data)) %>%
  # we have to slice off the first and last values because they go to 
  # infinity on the prior, which creates problems when computing the 
  # denominator `sum(likelihood * prior)` (i.e., p(D))
  slice(2:999) %>% 
  mutate(posterior = likelihood * prior / sum(likelihood * prior))
...
```{r, fig.width = 4.5, fig.height = 7, warning = FALSE, message = FALSE}
p1 <-
  d %>% 
...
We can calculate the beta parameters for the posterior using the formula $\operatorname{beta} \left( \theta \mid z + \alpha, \; N - z + \beta \right)$.
...
$$p \left( z, N \mid M_\text{null} \right) = \theta_\text{null}^z \left( 1 - \theta_\text{null} \right)^{(N - z)},$$
...
$$p \left( z, N \mid M_\text{alt} \right) = \frac{\operatorname{beta} \left( z + \alpha_\text{alt} , \; N - z + \beta_\text{alt} \right)}{\operatorname{beta}  \left( \alpha_\text{alt}, \; \beta_\text{alt} \right)}.$$
...
### The Bayes' Factor Can Accept a Null with Poor Precision
...
d <-
  ...
  mutate(prior      = dbeta(x = theta, shape1 = epsilon, shape2 = epsilon),
         likelihood = bernoulli_likelihood(theta = theta,
                                           data = trial_data)) %>%
  # like before, we have to slice off the first and last values because 
  # they go to infinity on the prior, which creats problems when computing 
  # the denominator `sum(likelihood * prior)` (i.e., p(D))
  slice(2:999) %>% 
  mutate(posterior = likelihood * prior / sum(likelihood * prior))
...
Kruschke discussed how we'd need $z = 1, \!200$ and $N = 2, \!400$ before the posterior HDIs would fit within a narrow ROPE like 0.48 and 0.52. Here's what that would look like based on the priors from Figure 12.4.
...
```{=latex}
\begin{align*}
n _{{\rm recalled}, ij} & \sim \operatorname{Binomial} \left( n = 20, \theta _{j} \right), \text{ in which} \\
\operatorname{logit} \left( \theta _j \right) &= \beta _{0, j} .
\end{align*}
```

In our equation, $\beta_{0, j}$ is the group-specific intercept within the logistic regression model. We'll use the $N(0, 1.5)$ prior for the intercept. Though it appears strongly regularizing in the log-odds space, it's quite flat on the $\theta$ space. If we wanted to be more conservative in the $\theta$ space, we might use something more like $N(0, 1)$.
...
fit12.1 <-
  brm(data = my_data, 
      ...
      seed = 12,
      file = "~/Desktop/DBDA/fits/fit12.01") 
```
...
fit12.2 <-
  brm(data = my_data, 
      ...
      iter = 3000, warmup = 1000, cores = 4, chains = 4,
      seed = 12,
      file = "~/Desktop/DBDA/fits/fit12.02") 
```

Here we'll compare the two models with the LOO.

```{r loo_fit12.1_fit12.2, message = FALSE}
...
```{r}
hypothesis(fit12.1, 
           "inv_logit_scaled(conditionDasKruschke) < 
           inv_logit_scaled(conditionBach)")
```
...
Let's see what happens when we test a different kind of directional hypothesis, $\theta_\text{Mozart} - \theta_\text{Bach} > 0$.

```{r}
hypothesis(fit12.1, 
           "inv_logit_scaled(conditionMozart) - 
           inv_logit_scaled(conditionBach) > 0")
```

Here we get an underwhelming BF of 1.49. The posterior probability that hypothesis versus its logical alternative is 0.59. Notice we no longer have a `*` in the `Star` column.
