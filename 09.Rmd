...
> The parameters at different levels in a hierarchical model are all merely parameters that coexist in a joint parameter space. We simply apply Bayes' rule to the joint parameter space, as we did for example when estimating two coin biases back in Figure 7.5, p. 167. To say it a little more formally with our parameters $\theta$ and $\omega$, Bayes' rule applies to the joint parameter space: $p \left( \theta, \omega \mid D \right) \propto p \left( D \mid \theta, \omega \right) p(\theta, \omega)$. What is special to hierarchical models is that the terms on the right-hand side can be factored into a chain of dependencies, like this:
>
> \begin{align*}
> p \left( \theta, \omega \mid D \right) &\propto p \left( D \mid \theta, \omega \right) \; p(\theta, \omega) \\
> &= p \left( D \mid \theta \right) \; p \left( \theta \mid \omega \right) \; p(\omega)
> \end{align*}
>
...
$$\theta \sim \operatorname{beta} \left( \omega(\kappa - 2) + 1, \; (1 - \omega)(\kappa - 2) + 1 \right).$$
...
tibble(k = seq(from = 10, to = 200, by = 10)) %>% 
  ...
  ggplot(aes(x = x, y = k,
             height = theta,
             group = k, fill = k)) +
  geom_vline(xintercept = 0.25, color = "grey85", size = 1/2) +
  geom_ridgeline(size = 1/5, color = "grey92", scale = 2) +
  scale_fill_viridis_c(expression(kappa), option = "A") +
  scale_y_continuous(expression(kappa), breaks = seq(from = 10, to = 200, 
        by = 10)) +
...
```{r, fig.width = 3.8, fig.height = 5, message = FALSE}
library(ggforce)
library(patchwork)
library(dplyr)
...
p2 <-
  tibble(x = c(0.5, 0.475, 0.26, 0.08, 0.06,
               0.5, 0.55, 0.85, 1.15, 1.2),
         y = c(1, 0.7, 0.6, 0.5, 0.2,
               1, 0.7, 0.6, 0.5, 0.2),
  ...
  annotate(geom = "text",
    ...
    size = 7, parse = TRUE, family = "Times", hjust = 0) +
  annotate(geom = "text",
           x = 1/3, y = 0.7,
           label = "'~'",
           size = 10, parse = TRUE, family = "Times") +
...
p5 <-
  tibble(x = 0:1,
         d = (dbinom(x, size = 1, prob = 0.6)) / max(dbinom(x, size = 1, 
                prob = 0.6))) %>% 
...
That is, the mode of $\operatorname{beta}(2, 2)$ is $0.5$.
...
```{=latex}
\begin{align}
p(\theta, \omega) &= p \left( \theta \mid \omega \right) \; p(\omega) \\
&= \operatorname{beta} \left( \theta \mid \omega (100 - 2) + 1, (1 - \omega) (100 - 2) + 1 \right) \\
&= \operatorname{beta} \left(\omega \mid 2, 2 \right)
\end{align}
```
...
d <-
  crossing(theta = parameter_space,
           omega = parameter_space) %>%
  mutate(prior = make_prior(theta, omega, alpha = 20, beta = 20, 
        kappa = 6)) %>% 
  mutate(prior = prior / sum(prior))
```
...
Now on to the posterior. Our first depiction will be the middle panel of the second row from the bottom of Figure 9.3. This will be $p \left( \theta, \omega \mid y \right)$.
...
```{r, fig.width = 3.8, fig.height = 5.2, message = FALSE}
p1 <-
  tibble(x = seq(from = 0.01, to = 0.99, by = 0.01),
       d = (dbeta(x, 2, 2)) / max(dbeta(x, 2, 2))) %>% 
  ggplot(aes(x = x, y = d)) +
...
d <-
  crossing(theta_1 = parameter_space,
           ...
           omega   = parameter_space) %>%
  mutate(prior = make_prior(theta_1, theta_2, omega, alpha = 2, beta = 2, 
        kappa = 5)) %>% 
  # here we normalize
  mutate(prior = prior / sum(prior))
...
> independence of the data across the two coins means that the data from coin 1 depend only on the bias in coin 1, and the data from coin 2 depend only on the bias in coin 2, which can be expressed formally as $p \left( y_1 \mid \theta_1, \theta_2 \right) = p \left( y_1 \mid \theta_1 \right)$ and $p \left( y_2 mid \theta_1, \theta_2 \right) = p \left( y_2 \mid \theta_2 \right)$. (p. 164)

The likelihood function for our two series of coin flips is then

$$p \left( D \mid \theta_1, \theta_2 \right) = \left[ \theta_1^{z_1} \left( 1 - \theta_1 \right) ^ {N_1 - z_1} \right] \left[ \theta_2^{z_2} \left(1 - \theta_2 \right) ^ {N_2 - z_2} \right].$$
...
```{r, fig.height = 3, fig.width = 3.25, message = FALSE, warning = FALSE}
p11 <-
  d %>% 
  group_by(theta_1, omega) %>% 
  summarise(prior = sum(prior)) %>% 
  ...
  annotate(geom = "text", x = 0.05, y = 0.925, 
           ...
           parse = TRUE, color = "white", hjust = 0) +
  labs(x = expression(theta[1]),
       y = expression(omega))
...
```{r, fig.height = 3, fig.width = 3.25, warning = FALSE, message = FALSE}
# D1: 3 heads, 12 tails
n <- 15
z <- 3
...
p23 <-
  ...
  geom_text(aes(size = size), 
            parse = TRUE, hjust = 0, show.legend = F) +
...
text <-
  tibble(x = 1,
         y = 10:8,
         label = c("Likelihood", "D1: 3 heads, 12 tails", 
                   "D2: 4 heads, 1 tail"),
         size = c(2, 1, 1))
...
```{r, fig.width = 3.85, fig.height = 5, message = FALSE}
# a beta density
p1 <-
  tibble(x = seq(from = 0.01, to = 0.99, by = 0.01),
         d = (dbeta(x, 2, 2)) / max(dbeta(x, 2, 2))) %>% 
...
p2 <-
  tibble(x = seq(from = 0, to = 5, by = 0.01),
         d = (dgamma(x, 1.75, 0.85) / max(dgamma(x, 1.75, 0.85)))) %>% 
  ggplot(aes(x = x, y = d)) +
  geom_area(fill = a_purple) +
  annotate(geom = "text",
           x = 2.5, y = 0.2,
...
tibble(shape = c(0.01, 1.56, 1, 6.25),
  ...
  scale_y_continuous(expression(p(kappa*"|"*s*","*r)), 
...
my_data <- read_csv("~/Desktop/DBDA/data.R/TherapeuticTouchData.csv")
...
```{r, fig.width = 4, fig.height = 3.25, message = FALSE, warning = FALSE}
my_data %>% 
  group_by(s) %>% 
...
fit9.1 <-
  brm(data = my_data,
      ...
      seed = 9,
      file = "~/Desktop/DBDA/fits/fit09.01")
```
...
The $N_{\rm eff}/N$ ratio values for our model parameters were excellent. Here's a numeric summary of the model:
...
draws_small %>% 
  ...
  mutate(name = factor(name, levels = c("theta[1]", "theta[14]", "theta[28]", 
        "theta[1] - theta[14]", "theta[1] - theta[28]", 
        "theta[14] - theta[28]"))) %>% 
...
```{r, fig.width = 6.5, fig.height = 2, warning = FALSE, message = FALSE}
p1 <-
  draws_small %>% 
...
```{r, fig.width = 3.25, fig.height = 2.5, warning = FALSE, message = FALSE}
...
sigmas %>% 
  ...
  stat_histinterval(point_interval = mode_hdi, .width = 0.95,
                    fill = a_purple, breaks = 40) +
  scale_x_continuous(expression(paste(sigma, " of ", 
        theta, " in a probability metric")),
                     breaks = labels$value,
                     labels = labels$label) +  
  scale_y_continuous(NULL, breaks = NULL) +
  theme_minimal_hgrid()
...
```{r, fig.width = 5.5, fig.height = 5, warning = FALSE, message = FALSE}
...
tibble(prior = rbeta(n = 100000, 1, 1)) %>% 
  ggplot(aes(x = prior)) +
  geom_histogram(fill = a_purple, color = "white", 
                 size = 0.2, binwidth = 0.05, boundary = 0) +
  scale_x_continuous(expression(omega), labels = c("0", "0.25", "0.5", 
        "0.75", "1")) +
  scale_y_continuous(NULL, breaks = NULL) +
  coord_cartesian(xlim = c(0, 1)) +
  theme_minimal_hgrid()
...
```{r prior_sim, cache = TRUE, fig.width = 6.5, fig.height = 4.875}
# define a function
r_norm <- function(i, n = 10000) {
...
tibble(sd = seq(from = 0.25, to = 3, by = 0.25)) %>% 
  ...
  # plot!
  ggplot(aes(x = prior)) +
  geom_histogram(fill = a_purple, color = "white", size = 0.2,
...
fit9.1_prior <-
  brm(data = my_data,
      ...
      seed = 9,
      sample_prior = "only",
      file = "~/Desktop/DBDA/fits/fit09.01_prior")
```
...
$$\theta \sim \operatorname{beta} \left[ \left(\omega(\kappa - 2) + 1 \right), (1 - \omega)(\kappa - 2) + 1) \right].$$
...
tibble(x = seq(from = 0, to = 1, by = .001)) %>%
  ...
  geom_area(fill = a_purple) +
  scale_y_continuous(expression(dbeta(theta*"|"*omega*", "*kappa)), 
        breaks = NULL) +
...
load("~/Desktop/DBDA/fits/09_fit_times.rda")
```
...
```{r, fig.width = 7.75, fig.height = 6.75, warning = FALSE, message = FALSE}
# half-normal density
p1 <-
...
tibble(x = 0:7) %>% 
  ggplot(aes(x = x, 
             y = (dbinom(x, size = 7, prob = 0.625)) / max(dbinom(x, 
        size = 7, prob = 0.625)))) +
  geom_col(fill = a_purple, width = 0.4) +
  annotate(geom = "text",
           x = 3.5, y = 0.2,
...
Here's a histogram of the $N_{\rm eff}/N$ ratios:
