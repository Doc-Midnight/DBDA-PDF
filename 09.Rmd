...
p2 <-
  ...
  annotate(geom = "text",
           x = 0, y = 0.125,
           label = "omega(italic(K) - 2) + 1*', 
           '*(1 - omega)(italic(K) - 2) + 1",
...
$$p(\theta, \omega) = p \left( \theta \mid \omega \right) \; p(\omega) = \operatorname{beta} \left(\theta \mid \omega (100 - 2) + 1, (1 - \omega) (100 - 2) + 1 \right) \; \operatorname{beta} \left(\omega \mid 2, 2 \right)$$
...
```{r, fig.width = 6.5, fig.height = 2, warning = F, message = F}
p1 <-
  draws_small %>% 
...
```{r prior_sim, cache = T, fig.width = 6.5, fig.height = 4.875}
# define a function
...
$$\theta \sim \operatorname{beta} \left(\omega(\kappa - 2) + 1 \right), (1 - \omega)(\kappa - 2) + 1).$$
