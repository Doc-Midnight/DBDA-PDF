...
tibble(theta = seq(from = 0, to = 1, length.out = 100)) %>% 
  ...
  geom_point(aes(x = omega, y = 0),
             size = 1.5, shape = 19) +
  annotate(geom = "text", x = 0.675, y = 0.4, 
...
```{r, fig.width = 10, fig.height = 2.75, warning = FALSE, message = FALSE}
library(tidybayes)

d %>% 
  ggplot(aes(x = theta, y = 0)) +
  stat_histinterval(point_interval = mode_hdi, .width = 0.95, breaks = 30) +
...
If you wanted a better sense of the phenomena, you could do a simulation. We'll make a custom simulation function to compute the modes from many random draws from our $\operatorname{beta} \left( \theta \mid 15, 7 \right)$ distribution, with varying $N$ values.
...
```{r sima, fig.height = 3.75, warning = FALSE, message = FALSE, cache = TRUE}
# we need an index of the values we set our seed with in our 
# `my_mode_simulation()` function
sim <-
  tibble(seed = 1:1000) %>% 
  group_by(seed) %>% 
...
sim %>% 
  ...
  stat_histinterval(.width = c(0.5, 0.95), breaks = 20, fill = "steelblue") +
  labs(title = expression("Variability of the mode for simulations of "*beta(theta*'|'*15*', '*7)*", the true mode of which is 0.7"),
       subtitle = "For each sample size, the dot is the median, the inner thick 
       line is the percentile-based 50% interval, and the outer thin line the 
       percentile-based 95% interval. Although the central tendency 
       approximates the true value for all three conditions, the variability of 
       the mode estimate is inversely related to the sample size.",
       x = "Mode", 
       y = NULL) +
...
tibble(x = 1:7,
  ...
  scale_x_continuous(expression(theta), breaks = 1:7) +
  scale_y_continuous(expression(p(theta)), 
...
for(from_idx in 1:nslots) {
  for(to_idx in 1:nslots) {
    acceptance_matrix[from_idx, to_idx] <- min(p_target[to_idx] / 
        p_target[from_idx], 1)
  }
}
...
p %>% 
  ...
  mutate(facet = factor(facet, 
    levels = c(str_c("italic(t)==", c(1:14, 99)), "target"))) %>% 
  ...
  scale_x_continuous(expression(theta), breaks = 1:7) +
  scale_y_continuous(expression(italic(p)(theta)), 
        expand = expansion(mult = c(0, 0.05))) +
  theme_cowplot() +
...
>
> This technique is profoundly useful when the target distribution $P(\theta)$ is a posterior proportional to $p \left( D \mid \theta \right) p(\theta)$. Merely by evaluating $p \left( D \mid \theta \right) p(\theta)$, without normalizing it by $p(D)$, we can generate random representative values from the posterior distribution. This result is wonderful because the method obviates direct computation of the evidence $p(D)$, which, as you'll recall, is one of the most difficult aspects of Bayesian inference. By using MCMC techniques, we can do Bayesian inference in rich and complex models. It has only been with the development of MCMC algorithms and software that Bayesian inference is applicable to complex data analysis, and it has only been with the production of fast and cheap computer hardware that Bayesian inference is accessible to a wide audience (page 152; *emphasis* in the original).
...
### Metropolis Algorithm Applied to a Bernoulli Likelihood and a Beta Prior
...
d %>% 
  ...
  stat_histinterval(point_interval = mode_hdi, .width = 0.95,
                    fill = "steelblue", slab_color = "white", outline_bars = TRUE,
...
### Summary of the Metropolis Algorithm
...
```{r betas, cache = TRUE, fig.height = 3}
set.seed(7)
...
```{r, fig.height = 3}
theta_sequence <- seq(from = 0, to = 1, by = 0.01)
...
d_prior %>% 
  ...
  mutate(marginal_likelihood = 
                 sum(prior_1 * prior_2 * likelihood_1 * likelihood_2)) %>% 
  # finally, the two-dimensional posterior
  mutate(posterior = (prior_1 * prior_2 * likelihood_1 * likelihood_2) / 
           marginal_likelihood) %>% 
  
  # plot!
  ggplot(aes(x = theta_1, y = theta_2, fill = posterior)) +
  geom_tile() +
  scale_fill_viridis_c(expression(italic(p)(theta[1]*', '*theta[2]*'|'*D)), 
                       option = "A") +
  scale_x_continuous(expression(theta[1]), expand = expansion(mult = 0)) +
  scale_y_continuous(expression(theta[2]), expand = expansion(mult = 0)) +
  coord_equal() +
...
$$p \left( \theta_1, \theta_2 \mid D \right) = \frac{p \left( D \mid \theta_1, \theta_2 \right) p \left( \theta_1, \theta_2 \right)}{p(D)}.$$
...
bivariate_bernoulli_likelihood <- function(theta1, data1, theta2, data2) {
  ...
  z2 <- sum(data2)
  n2 <- length(data2)
  p_data_given_theta <- (theta1^z1 * (1 - theta1)^(n1 - z1)) * (theta2^z2 * 
        (1 - theta2)^(n2 - z2))
...
# we also need to update how we define the relative probability of the 
# target distribution
  ...
  for (t in 1:(traj_length - 1)) {
    ...
    prob_accept <- min(1,
        ...
        current_position2 + proposed_jump2, theta_2_data)
          / target_rel_prob(current_position1, theta_1_data, 
                            current_position2, theta_2_data))
...
fit7.1a <-
  brm(data = d, 
      ...
      seed = 7,
      file = "~/Desktop/DBDA/fits/fit07.01a")
```
...
fit7.1b <-
  brm(data = d, 
      ...
      iter = 3000, warmup = 500, cores = 3, chains = 3,
      seed = 7,
      file = "~/Desktop/DBDA/fits/fit07.01b")
```
...
```{r, fig.height = 2.25, fig.width = 6, warning = FALSE}
# combine the posterior samples from the two models
draws <-
  bind_rows(
    draws.a %>% transmute(`theta[1]` = b_y1_Intercept, 
                          `theta[2]` = b_y2_Intercept),
    draws.b %>% transmute(`theta[1]` = b_z1_Intercept, 
                          `theta[2]` = b_z2_Intercept)
  ) %>% 
...
draws %>% 
  ...
  stat_histinterval(point_interval = mode_hdi, .width = 0.95,
                    fill = "steelblue2", 
                    slab_color = "steelblue4", outline_bars = TRUE,
                    breaks = 40, normalize = "panels") +
...
fit7.2 <-
  brm(data = d, 
      ...
      seed = 7,
      file = "~/Desktop/DBDA/fits/fit07.02")
```
...
```{r, fig.width = 6, fig.height = 2, warning = FALSE}
draws <- as_draws_df(fit7.2, inc_warmup = TRUE)
...
p1 <- 
  ...
  scale_y_continuous(breaks = 6:8 / 10, limits = c(0.53, 0.84)) +
  labs(title = "Lag 1",
       x = "Index 1:70",
       y = expression(theta))
...
p2 <-
  ...
  scale_y_continuous(NULL, labels = NULL, 
                     breaks = 6:8 / 10, limits = c(0.53, 0.84)) +
  labs(title = "Lag 5",
       x = "Index 1:70")
