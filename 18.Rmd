...
$$y \sim \operatorname{Normal} \left( \beta_0 + \beta_1 x_1 + \beta_2 x_2, \sigma \right).$$
...
```{r, fig.width = 3.5, fig.height = 3.25, warning = FALSE, message = FALSE}
theme_set(
  theme_linedraw() +
...
```{r, fig.width = 6, fig.height = 5.5, warning = FALSE, message = FALSE}
p1 <-
  d %>% 
  ggplot(aes(x = x_1, y = y)) +
  geom_path(data = grid,
...
> Real data often have correlated predictors. For example, consider trying to predict a state's average high-school SAT score on the basis of the amount of money the state spends per pupil. If you plot only mean SAT against money spent, there is actually a *decreasing* trend$\ldots$ (p. 513; *emphasis* in the original).

Before we remake Figure 18.3 to examine that decreasing trend, we'll need to load the data from [@guber1999getting].

```{r, message = FALSE}
my_data <- read_csv("~/Desktop/DBDA/data.R/Guber1999data.csv")
...
```{r, fig.width = 6.75, fig.height = 5, message = FALSE}
# normal density
p1 <-
...
p3 <-
  ...
  annotate(geom = "text",
           x = c(.35, 1.3), y = .5,
           label = "'~'",
           size = 10, family = "Times", parse = TRUE) +
  xlim(0, 2) +
  theme_void()
...
p5 <-
  ...
  geom_text(size = 7, parse = TRUE, family = "Times") +
...
fit18.1 <-
  brm(data = my_data,
      ...
      seed = 18,
      file = "~/Desktop/DBDA/fits/fit18.01")
```
...
stat_wilke <- function(height = 1.25, point_size = 5, ...) {
  ...
  list(
    ...
    stat_halfeye(.width = 0, point_interval = mode_qi,
                 ...
                 ...),
    # fill settings
    scale_fill_ramp_discrete(range = c(1, .4), na.translate = FALSE),
...
color_scheme_set(c("steelblue4", "steelblue4", "steelblue4", "steelblue4", 
...
fit18.2 <-
  brm(data = my_data,
      ...
      sample_prior = "yes",
      file = "~/Desktop/DBDA/fits/fit18.02")
```
...
color_scheme_set(c("steelblue4", "steelblue4", "chocolate3", "steelblue4", 
                   "steelblue4", "steelblue4"))
...
Looks like HMC made a big difference. The $N_{\rm eff}/N$ ratios weren't terrible, either.
...
The `brms::vcov()` function returns a variance/covariance matrix (or a correlation matrix when you set `correlation = TRUE`) of the population-level parameters (i.e., the fixed effects). It returns the values to a decadent level of precision, so we'll simplify the output with `round()`.
...
```{r, fig.width = 6.5, fig.height = 2, warning = FALSE}
draws %>% 
  pivot_longer(b_Intercept:b_prop_not_take_z) %>% 
  # this line isn't necessary, but it does allow us to arrange the 
  # parameters on the y-axis
  mutate(name = factor(name, 
    levels = c("b_prop_not_take_z", "b_prcnt_take_z", 
      "b_spend_z", "b_Intercept"))) %>% 
...
```{r}
make_beta_0 <- function(zeta_0, zeta_1, zeta_2, zeta_3, sd_x_1, sd_x_2, 
  sd_x_3, sd_y, m_x_1, m_x_2, m_x_3,  m_y) {
  sd_y * zeta_0 + m_y - sd_y * ((zeta_1 * m_x_1 / sd_x_1) + 
    (zeta_2 * m_x_2 / sd_x_2) + (zeta_3 * m_x_3 / sd_x_3))
}
```
...
```{r, fig.width = 7, fig.height = 6.75, warning = FALSE, message = FALSE}
draws %>% 
  set_names("Intercept~(beta[0])", "Spend~(beta[1])", "Percent~Take~(beta[2])", 
            "Percent~Not~Take~(beta[3])", "sigma", "log10(nu)") %>% 
  ggpairs(upper = list(continuous = my_upper),
          diag = list(continuous = my_diag),
          lower = list(continuous = my_lower),
          labeller = label_parsed) +
...
\mu & = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_{1 \times 2} x_1 x_2 \\
    & = \beta_0 + \underbrace{ \left( \beta_1 + \beta_{1 \times 2} x_2 \right)}_{\text{slope of } x_1} x_1 + \beta_2 x_2 \\
    & = \beta_0 + \beta_1 x_1 + \underbrace{ \left( \beta_2 + \beta_{1 \times 2} x_1 \right)}_{\text{slope of } x_2} x_2.
\end{align*}
...
With **brms**, you can specify an interaction with either the `x_i*x_j` syntax or the `x_i:x_j` syntax. I typically use `x_i:x_j`. It's often the case that you can just make the interaction term right in the model formula. But since we're fitting the model with standardized predictors and then using Kruschke's equations to convert the parameters back to the un-standardized metric, it seems easier to make the interaction term in the data, first.
...
fit18.3 <-
  brm(data = my_data,
      ...
      stanvars = stanvar(1/29, name = "one_over_twentynine"),
      seed = 18,
      file = "~/Desktop/DBDA/fits/fit18.03")
```
...
Let's convert the posterior draws to the un-standardized metric.
...
p3 <-
  ...
  ggplot(aes(x = x, y = (dnorm(x)) / max(dnorm(x)))) +
  geom_area(fill = "steelblue4", color = "steelblue4", alpha = 0.6) +
  annotate(geom = "text",
           x = 0, y = 0.2,
           label = "normal",
           size = 7) +
...
p5 <-
  ...
  annotate(geom = "text",
           ...
           label = c("'~'", "'~'", "italic(j)"),
           size = c(10, 10, 7), family = "Times", parse = TRUE) +
  xlim(0, 2) +
  theme_void()
...
fit18.4 <-
  update(fit18.1, 
         newdata = my_data,
         formula = satt_z ~ 1 + prcnt_take_z + spend_z + x_rand_1 + x_rand_2 + 
           x_rand_3 + x_rand_4 + x_rand_5 + x_rand_6 + x_rand_7 + x_rand_8 + 
           x_rand_9 + x_rand_10 + x_rand_11 + x_rand_12,
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.04")
```
...
```{r}
make_beta_0 <- 
  function(zeta_0, zeta_1, zeta_2, zeta_3, zeta_4, zeta_5, zeta_6, zeta_7, 
           zeta_8, zeta_9, zeta_10, zeta_11, zeta_12, zeta_13, zeta_14, 
           sd_x_1, sd_x_2, sd_x_3, sd_x_4, sd_x_5, sd_x_6, sd_x_7, sd_x_8, 
           sd_x_9, sd_x_10, sd_x_11, sd_x_12, sd_x_13, sd_x_14, sd_y, 
           m_x_1, m_x_2, m_x_3, m_x_4, m_x_5, m_x_6, m_x_7, m_x_8, m_x_9, 
           m_x_10, m_x_11, m_x_12, m_x_13, m_x_14, m_y) {
    sd_y * zeta_0 + m_y - sd_y * ((zeta_1 * m_x_1 / sd_x_1) + 
                                    (zeta_2 * m_x_2 / sd_x_2) + 
                                    (zeta_3 * m_x_3 / sd_x_3) + 
                                    (zeta_4 * m_x_4 / sd_x_4) + 
...
draws %>% 
  ...
  stat_pointinterval(point_interval = mode_hdi, .width = 0.95, point_size = 4,
...
fit18.5 <-
  update(fit18.4, 
         ...
         prior = c(prior(normal(0, 2), class = Intercept),
                   ...
                   prior(exponential(one_over_twentynine), class = nu)),
         seed = 18,
      file = "~/Desktop/DBDA/fits/fit18.05")
```
...
```{r fit18.6, message = FALSE}
fit18.6 <-
  update(fit18.1,
         formula = satt_z ~ 1 + spend_z,
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.06")

fit18.7 <-
  update(fit18.1,
         formula = satt_z ~ 1 + prcnt_take_z,
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.07")
...
fit18.8 <-
  brm(data = my_data,
      ...
      stanvars = stanvar(1/29, name = "one_over_twentynine"),
      seed = 18,
      file = "~/Desktop/DBDA/fits/fit18.08")
```
...
as_draws_df(fit18.1) %>% 
  transmute(Spend     = make_beta_j(zeta_j = b_spend_z,
            ...
            PrcntTake = make_beta_j(zeta_j = b_prcnt_take_z,
                                    ...
                                    sd_y   = sd_y)) %>% 
  pivot_longer(everything()) %>% 
  # within `bind_rows()`, we extract and wrangle the posterior draws 
  # from `fit18.7` and them insert them below those from `fit18.1`
  bind_rows(
    as_draws_df(fit18.7) %>%
...
### The Inclusion Probability is Strongly Affected by Vagueness of the Prior

To follow along, let's fit the models with the updated $SD = 1$ on the $\beta_{1+}$ priors code.

```{r fit18.9, message = FALSE}
fit18.9 <-
  update(fit18.1,
         ...
         stanvars = stanvar(1/29, name = "one_over_twentynine"),
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.09")

fit18.10 <-
  update(fit18.9,
         formula = satt_z ~ 1 + spend_z,
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.10")

fit18.11 <-
  update(fit18.9,
         formula = satt_z ~ 1 + prcnt_take_z,
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.11")
...
fit18.12 <-
  update(fit18.8,
         prior = c(prior(normal(0, 1), class = Intercept),
                   ...
                   prior(exponential(one_over_twentynine), class = nu)),
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.12")
```

And now we'll fit the models with the updated $SD = 10$.

```{r fit18.13, message = FALSE}
fit18.13 <-
  update(fit18.9,
         prior = c(prior(normal(0, 10), class = Intercept),
                   ...
                   prior(exponential(one_over_twentynine), class = nu)),
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.13")

fit18.14 <-
  update(fit18.13,
         formula = satt_z ~ 1 + spend_z,
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.14")

fit18.15 <-
  update(fit18.13,
         formula = satt_z ~ 1 + prcnt_take_z,
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.15")
...
fit18.16 <-
  update(fit18.12,
         prior = c(prior(normal(0, 10), class = Intercept),
                   ...
                   prior(exponential(one_over_twentynine), class = nu)),
         seed = 18,
         file = "~/Desktop/DBDA/fits/fit18.16")
```
...
mw %>% 
  ...
  labs(x = "Stacking weight",
       y = expression(paste("Models defined by Kruschke's ", 
        delta, " notation"))) +
  coord_cartesian(xlim = c(0, 1)) +
  theme(panel.grid = element_blank()) +
  panel_border() +
...
as_draws_df(fit18.9) %>% 
  ...
  pivot_longer(everything()) %>% 
  # within `bind_rows()`, we extract and wrangle the posterior draws 
  # from `fit18.11` and then insert them below those from `fit18.9`
  bind_rows(
...
tibble(x = seq(from = 0, to = 65, length.out = 1000)) %>% 
  ggplot(aes(x = x, y = dgamma(x, shape = p[1], rate = p[2]))) +
  geom_area(color = "steelblue4", fill = "steelblue4", alpha = 0.6) +
  scale_y_continuous(NULL, breaks = NULL) +
...
fit18.1111 <-
  brm(data = my_data,
      ...
      prior = c(prior(normal(0, 2), class = Intercept),
                ...
                set_prior("target += gamma_lpdf(tau | my_shape, my_rate)", 
                          check = FALSE),
                prior(normal(0, 1), class = sigma),
                prior(exponential(one_over_twentynine), class = nu)),
      chains = 4, cores = 4,
      ...
      stanvars = stanvars,
      seed = 18,
      file = "~/Desktop/DBDA/fits/fit18.1111")

fit18.0111 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + prcnt_take_z + stu_tea_rat_z + salary_z,
         file = "~/Desktop/DBDA/fits/fit18.0111")

fit18.1011 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + spend_z + stu_tea_rat_z + salary_z,
         file = "~/Desktop/DBDA/fits/fit18.1011")
 
fit18.1101 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + spend_z + prcnt_take_z + salary_z,
         file = "~/Desktop/DBDA/fits/fit18.1101")

fit18.1110 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + spend_z + prcnt_take_z + stu_tea_rat_z,
         file = "~/Desktop/DBDA/fits/fit18.1110")

fit18.0011 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + stu_tea_rat_z + salary_z,
         file = "~/Desktop/DBDA/fits/fit18.0011")

fit18.0101 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + prcnt_take_z + salary_z,
         file = "~/Desktop/DBDA/fits/fit18.0101")

fit18.0110 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + prcnt_take_z + stu_tea_rat_z,
         file = "~/Desktop/DBDA/fits/fit18.0110")

fit18.1001 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + spend_z + salary_z,
         file = "~/Desktop/DBDA/fits/fit18.1001")

fit18.1010 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + spend_z + stu_tea_rat_z,
         file = "~/Desktop/DBDA/fits/fit18.1010")

fit18.1100 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + spend_z + prcnt_take_z,
         file = "~/Desktop/DBDA/fits/fit18.1100")

fit18.0001 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + salary_z,
         file = "~/Desktop/DBDA/fits/fit18.0001")

fit18.0010 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + stu_tea_rat_z,
         file = "~/Desktop/DBDA/fits/fit18.0010")

fit18.0100 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + prcnt_take_z,
         file = "~/Desktop/DBDA/fits/fit18.0100")

fit18.1000 <-
  update(fit18.1111,
         formula = satt_z ~ 1 + spend_z,
         file = "~/Desktop/DBDA/fits/fit18.1000")

fit18.0000 <-
  update(fit18.1111,
         formula = satt_z ~ 1                                                    ,
         file = "~/Desktop/DBDA/fits/fit18.0000")
```
...
# first, we need to redefine `sd_x_3` and `sd_x_4` in terms of our two 
# new predictors
...
as_draws_df(fit18.0100) %>% 
  ...
  mutate(name = factor(name, levels = c("Spend", "PrcntTake", "StuTeaRat", 
    "Salary")),
         fit = factor(fit, levels = c("fit18.0100", "fit18.0111", 
          "fit18.1100", "fit18.0001", "fit18.1000", "fit18.0110"))) %>%
...
```{r 18_pp_average_1, cache = TRUE, warning = FALSE}
...
pp <-
  # the first things we feed into `pp_average()` are the `brm()` fits over  
  # which we would like to average
  pp_average(fit18.1111, fit18.0111, fit18.1011, fit18.1101, fit18.1110, 
             fit18.0011, fit18.0101, fit18.0110, fit18.1001, fit18.1010, 
             fit18.1100, fit18.0001, fit18.0010, fit18.0100, fit18.1000, 
             fit18.0000,
  # here we tell it to evaluate the posterior over these predictor values
             newdata = nd, 
  # we can get the mean trends using the "fitted" method
             method = "fitted",
  # by `robust`, we mean we'd like the Estimate in terms of posterior 
...
```{r 18_pp_average_2, cache = TRUE, warning = FALSE}
# how many points on the x-axis?
n_points <- 30
...
pp %>% 
  ...
  mutate(x = seq(from = -3, to = 3, length.out = n_points) %>% rep(., times = 4),
         predictor = rep(c("Spend_z", "PrcntTake_z", "StuTeaRat_z", 
          "Salary_z"), each = n_points)) %>% 
