...
bar <-
  ...
  mutate(draw = map2(mu, sigma, ~rnorm(10000, mean = .x, sd = .y))) %>% 
  unnest(draw) %>% 
...
bar %>% 
  ...
  geom_col(width = 0.5, alpha = 0.85, fill = sl[7]) +
...
```{r, warning = F, message = F}
library(readr)
my_data_1 <- read_csv("~/Desktop/DBDA/data.R/OrdinalProbitData-1grp-1.csv")
...
fit23.1 <-
  brm(data = my_data_1,
      ...
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.01")
```
...
draws %>% 
  ...
  scale_color_scico_d(palette = "lajolla", begin = 0.25) +
  ylab("mean threshold") +
...
Kruschke didn't do this in the text, but it might be informative to plot the probability distributions for the seven categories from `Y` (i.e., $p(y = k \mid \mu = 0, \sigma = 1, \{ \theta_i \})$).
...
```{r, warning = F, message = F}
my_data_2 <- read_csv("~/Desktop/DBDA/data.R/OrdinalProbitData-1grp-2.csv")
```
...
fit23.2 <-
  update(fit23.1, 
         ...
         iter = 3000, warmup = 1000, chains = 4, cores = 4,
         seed = 23,
         file = "~/Desktop/DBDA/fits/fit23.02")
```
...
fit23.3 <-
  brm(data = my_data_1,
      ...
      stanvars = stanvars, 
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.03")
```
...
fit23.4 <-
  update(fit23.3,
         ...
         stanvars = stanvars, 
         seed = 23,
         file = "~/Desktop/DBDA/fits/fit23.04")
```
...
```{r, message = F}
my_data <- read_csv("~/Desktop/DBDA/data.R/OrdinalProbitData1.csv")
...
fit23.5 <-
  brm(data = my_data,
      ...
      iter = 3000, warmup = 1000, chains = 4, cores = 4,
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.05")
```
...
tibble(X     = LETTERS[1:2],
  ...
  scale_fill_scico_d(palette = "lajolla", begin = 0.33, end = 0.67) +
...
fit23.6 <-
  brm(data = my_data,
      ...
      stanvars = stanvars, 
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.06")
```
...
fit23.7 <-
  brm(data = d,
      ...
      iter = 3000, warmup = 1000, chains = 4, cores = 4,
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.07")
```
...
fit23.8 <-
  brm(data = d,
      ...
      stanvars = stanvars, 
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.08")
```
...
```{r, message = F}
my_data <- read_csv("~/Desktop/DBDA/data.R/OrdinalProbitData-LinReg-2.csv")
...
fit23.9 <-
  brm(data = my_data,
      ...
      iter = 3000, warmup = 1000, chains = 4, cores = 4,
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.09")
```
...
Recall that our thresholds, $\theta_1,...,\theta_{K-1}$, in conjunction with the standard normal density, give us the probability of a given `Y` value, given `X_s` (i.e., $p(y = k \mid \mu, \sigma, \{ \theta_j \})$, where $\mu$ is conditional on $x$). This plot returned the fitted lines of those conditional probabilities, each depicted by the posterior mean and percentile-based 95% intervals. At lower values of `X_s`, lower values of `Y` are more probable. At higher values of `X_s`, higher values of `Y` are more probable.
...
f %>% 
  ...
  scale_y_reverse(NULL, position = "right", limits = c(1, 0), expand = c(0, 0),
                  breaks = c(1, 0.5, 0), labels = c("1", ".5", "0")) +
  coord_flip() +
...
fit23.10 <-
  brm(data = my_data,
      ...
      stanvars = stanvars, 
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.10")
```
...
```{r, message = F}
my_data <- read_csv("~/Desktop/DBDA/data.R/HappinessAssetsDebt.csv")
...
my_data %>% 
  ...
  geom_jitter(alpha = 1/4, height = 0.25, color = sl[9]) +
...
fit23.11 <-
  brm(data = my_data,
      ...
      iter = 3000, warmup = 1000, chains = 4, cores = 4,
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.11")
```
...
fit23.12 <-
  brm(data = my_data,
      ...
      stanvars = stanvars, 
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.12")
```
...
```{r, message = F}
my_data <- read_csv("~/Desktop/DBDA/data.R/Movies.csv")
...
my_data %>% 
  ...
  geom_point(aes(color = Rating),
             size = 3) +
  geom_text(size = 3) +
  scale_color_scico_d(palette = "lajolla", begin = 0.15, end = 0.7)
```
...
fit23.13 <-
  brm(data = my_data,
      ...
      iter = 3000, warmup = 1000, chains = 4, cores = 4,
      seed = 23,
      file = "~/Desktop/DBDA/fits/fit23.13")
```
...
draws %>% 
  ...
  geom_text(data = my_data,
            aes(label = Rating)) +
  scale_color_scico_d(expression(theta), palette = "lajolla", begin = 0.25, labels = 1:6) +
...
```{r, message = F}
my_data <- read_csv("~/Desktop/DBDA/data.R/OrdinalProbitData-Movies.csv")
...
fit23.14 <-
  update(fit23.13,
         ...
         iter = 3000, warmup = 1000, chains = 4, cores = 4,
         seed = 23,
         file = "~/Desktop/DBDA/fits/fit23.14")
```
...
plot(ce, plot = FALSE)[[1]] +
  scale_fill_scico_d(palette = "lajolla", begin = 0.25) +
  scale_color_scico_d(palette = "lajolla", begin = 0.25) +
  scale_x_continuous(expand = c(0, 0)) +
...
The cumulative-normal model makes posterior predictions for the probabilities of the $K$ categories in the criterion variable by computing $p (y \mid \mu (x), \sigma, \{ \theta_k \} )$ in each step in the HMC chain. In this equation, $\mu (x) = \beta_0 + \sum_j \beta_j x_j$. Though recall that with our **brms** parameterization, we have $\beta_0$ fixed at 0. Kruschke framed part of his discussion in this chapter in terms of a single-predictor model, such as was entertained in Figure 23.8. Recall that corresponds to our `fit11`. Here's that `formula`.
...
In this section, Kruschke mentioned extensions of this class of models might include using the cumulative $t$ function to handle outliers or adding a guessing parameter. Full disclosure: I have not fit models like these. Based on my knowledge of **brms**, I suspect they're possible. For insights how, you might review Bürkner's [-@Bürkner2021Define] [*Define custom response distributions with brms*](https://cran.r-project.org/web/packages/brms/vignettes/brms_customfamilies.html) and his [-@Bürkner2021Non_linear] [*Estimating non-linear models with brms*](https://cran.r-project.org/web/packages/brms/vignettes/brms_nonlinear.html) vignettes.
