...
$$p \left( \{ y_i \} \mid \theta \right) = \theta^z \cdot (1 - \theta) ^ {N - z},$$

where $z$ is the number of 1's in the data (i.e., heads in a series of coin flips) and the sole parameter a given observation will be a 1 is $\theta$ (i.e., the probability; $p \left( y_i = 1 \mid \theta \right)$).
...
>
> In principle, we could use any probability density function supported on the interval [0, 1]. When we intend to apply Bayes' rule (Equation 5.7, p. 106), however, there are two desiderata for mathematical tractability. First, it would be convenient if the product of $p \left(y \mid \theta \right)$ and $p(\theta)$, which is in the numerator of Bayes' rule, results in a function of the same form as $p(\theta)$... Second, we desire the denominator of Bayes' rule (Equation 5.9, p. 107), namely $\int \text d \; \theta p \left( y \mid \theta \right) p(\theta)$, to be solvable analytically. This quality also depends on how the form of the function $p(\theta)$ relates to the form of the function $p \left( y \mid \theta \right)$. When the forms of $p \left( y \mid \theta \right)$ and $p(\theta)$ combine so that the posterior distribution has the same form as the prior distribution, then $p(\theta)$ is called a *conjugate prior* for $p \left( y \mid \theta \right)$. (Page 127; *emphasis* in the original.)
...
```{r}
beta_param <- betaABfromModeKappa(mode = 0.25, kappa = 4)
...
$$p \left( \theta \mid z, N \right) = \frac{p \left( z, N \mid \theta \right) \; p(\theta)}{p(z, N)}.$$

### The Posterior is Compromised of the Prior and the Likelihood
...
line <-
  tibble(theta = c(0.212 + 0.008, 0.788 - 0.008, 0.114 + 0.004, 0.497 - 0.005),
         value = rep(c (0.51, 0.66), each = 2),
         xintercept = c (0.212, 0.788, 0.114, 0.497),
         name = rep(c("Prior (beta)", "Posterior (beta)"), each = 2)) %>% 
  mutate(name = factor(name, levels = levels))
  
# the data for the annotation
text <-
  tibble(theta = c (0.5, 0.3),
         value = c (0.8, 1.125),
         label = "95% HDI",
...
In the first paragraph of this section, Kruschke suggested we consider a beta prior with a mode of $\omega = 0.5$ and an effective sample size $\kappa = 500$. We might use Kruschke's `betaABfromModeKappa()` function to use those values to compute the corresponding $a$ and $b$ parameters for the beta distribution.

```{r}
betaABfromModeKappa(mode = 0.5, kappa = 500)
```

Confusingly, Kruschke switched from $\operatorname{beta(250, 250)}$ in the prose to $\operatorname{beta(100, 100)}$ in Figure 6.4.a, which he acknowledged in his [Corrigenda](https://sites.google.com/site/doingbayesiandataanalysis/corrigenda). We'll stick with $\operatorname{beta(100, 100)}$, which corresponds to $\omega = 0.5$ and $\kappa = 200$.

```{r}
betaABfromModeKappa(mode = 0.5, kappa = 200)
```
...
```{r}
# define the prior
beta_param <- betaABfromModeKappa(mode = 0.5, kappa = 200)
...
d %>% 
  ...
  geom_area(data = . %>% filter(theta > prior_hdi[1] & theta < prior_hdi[2]),
            fill = "steelblue") +
  geom_segment(x = prior_hdi[1] + 0.005, xend = prior_hdi[2] - 0.005,
               y = 1.8, yend = 1.8,
               arrow = arrow(length = unit (0.15,"cm"), 
                             ends = "both", 
                             type = "closed"),
               color = "white") +
  annotate(geom = "text", x = 0.5, y = 3.5, 
           label = "95% HDI") +
...
d %>% 
  ...
  geom_area(data = . %>% filter(theta > post_hdi[1] & theta < post_hdi[2]),
            fill = "steelblue") +
  geom_segment(x = post_hdi[1] + 0.005, xend = post_hdi[2] - 0.005,
               y = 2, yend = 2,
               arrow = arrow(length = unit (0.15, "cm"), 
                             ends = "both", 
                             type = "closed"),
               color = "white") +
  annotate(geom = "text", x = 0.532, y = 3.5, 
           label = "95% HDI") +
...
$$\omega_\text{beta} = \frac{a - 1}{a + b - 2}$$
...
For the next example, we consider the probability a professional basketball player will make free a throw. We have the same likelihood based on 17 successes our of 20 trials, but this time our prior is based on $\omega = 0.75$ and $\kappa = 25$. Here we update those values and our `d` data for the plot.
...
d %>% 
  ...
  geom_segment(x = post_hdi[1] + 0.005, xend = post_hdi[2] - 0.005,
               ...
               arrow = arrow(length = unit (0.15, "cm"), 
                             ...
                             type = "closed"),
               color = "white") +
  annotate(geom = "text", x = 0.797, y = 2, 
           label = "95% HDI", color = "white") +
  labs(title = "Posterior (beta)",
       x = expression(theta),
...
#### Figure 6.5 in Kruschke's Style
...
BernGrid = function( Theta , pTheta , Data , plotType=c("Points","Bars")[2] ,
  ...
  par( mar = c(3, 3, 1, 0) , mgp = c(2, 0.7, 0), 
       mai = c(0.5, 0.5, 0.3, 0.1)) # margins
  cexAxis = 1.33
  cexLab = 1.75
  ...
  plot( Theta[thinIdx] , pDataGivenTheta[thinIdx] , type=plotType , 
        ...
        cex.lab=cexLab, main="Likelihood" , cex.main=1.5 , col="skyblue" )
  if ( z > 0.5*N ) { textx = 0 ; textadj = c(0,1) }
  ...
  plot( Theta[thinIdx] , pThetaGivenData[thinIdx] , type=plotType , 
        ...
        cex.lab=cexLab, main="Posterior" , cex.main=1.5 , col="skyblue" )
  if ( showCentTend != "None" ) {
    if ( showCentTend == "Mean" ) {
...
#### Figure 6.5 in the **tidyverse** Style
